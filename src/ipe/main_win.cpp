...
static void setup_win_config(lua_State * L, const char * var, int folderId) {
    String s;
...
static HACCEL makeAccel(lua_State * L, int arg) {
	...
	a.fVirt = (((key & 0x10000) ? FALT : 0) | ((key & 0x20000) ? FCONTROL : 0)
		   | ((key & 0x40000) ? FSHIFT : 0) | ((key & 0x80000) ? 0 : FVIRTKEY));
	accel.push_back(a);
