...
static void buildButton(std::vector<short> & t, UINT flags, int id, int h, int x,
			const char * s) {
...
static void deleteItems(HWND lv, SData * d, bool cut) {
    int n = d->pages.size();
    if (cut) d->cutlist.clear();
    for (int i = n - 1; i >= 0; --i) {
	if (ListView_GetItemState(lv, i, LVIS_SELECTED) & LVIS_SELECTED) {
	    if (cut) d->cutlist.insert(d->cutlist.begin(), d->pages[i]);
...
static void showPopup(HWND parent, POINT pt, int index, SData * d, HWND lv) {
    ...
    int result = TrackPopupMenu(hMenu, TPM_NONOTIFY | TPM_RETURNCMD | TPM_RIGHTBUTTON,
...
static void handleResize(HWND hwnd) {
    ...
    int b = 16 * dpi / 96;
    MoveWindow(h, b, b, rc.right - rc.left - b - b, rc.bottom - rc.top - b - b - b - bh,
	       TRUE);
...
static INT_PTR CALLBACK dialogProc(HWND hwnd, UINT message, WPARAM wParam,
    ...
    case WM_SIZE: handleResize(hwnd); return TRUE;
    case WM_CLOSE: EndDialog(hwnd, -1); return TRUE;
    default: return FALSE;
    }
}
...
int AppUi::pageSorter(lua_State * L, Document * doc, int pno, int width, int height,
	...
	sData.hImageList =
	    ImageList_Create(thumbWidth, r.height(), ILC_COLOR32, p->countViews(), 4);
	...
	sData.hImageList =
	    ImageList_Create(thumbWidth, r.height(), ILC_COLOR32, doc->countPages(), 4);
    ...
    // Page sorter control
    if (t.size() % 2 != 0) t.push_back(0);
    buildFlags(t, WS_CHILD | WS_VISIBLE | LVS_ICON | LVS_AUTOARRANGE | WS_VSCROLL
		      | WS_BORDER);
    t.push_back(BORDER);
    t.push_back(BORDER);
    ...
    int res = DialogBoxIndirectParamW(nullptr, (LPCDLGTEMPLATE)t.data(), nullptr,
				      dialogProc, (LPARAM)&sData);
