...
static int appui_update(lua_State * L) {
	...
	canvas->invalidate(int(bl.x - 1), int(bl.y - 1), int(tr.x - bl.x + 2),
			   int(tr.y - bl.y + 2));
    }
    return 0;
}
...
static void check_rgb(lua_State * L, int i, int & r, int & g, int & b) {
    ...
    luaL_argcheck(L, (0 <= r && r <= 1000 && 0 <= g && g <= 1000 && 0 <= b && b <= 1000),
		  2, "color components must be between 0.0 and 1.0");
}
...
static int appui_setCursor(lua_State * L) {
	...
	static const char * const cursor_names[] = {"standard", "hand", "cross", nullptr};
...
static int appui_selecttool(lua_State * L) {
    ...
    SelectTool * tool =
	new SelectTool(canvas, page, view, selectDistance, nonDestructive);
    canvas->setTool(tool);
    return 0;
}

static int appui_transformtool(lua_State * L) {
    static const char * const option_names[] = {"translate", "scale", "stretch",
						"rotate",    "shear", nullptr};
    CanvasBase * canvas = check_canvas(L, 1);
    Page * page = check_page(L, 2)->page;
    int view = check_viewno(L, 3, page);
    ...
    IpeTransformTool * tool = new IpeTransformTool(
	canvas, page, view, TransformTool::TType(type), withShift, L, method);
    if (tool->isValid()) {
	canvas->setTool(tool);
...
static int shapetool_setshape(lua_State * L) {
    ...
    int which = 0;
    if (!lua_isnoneornil(L, 2)) which = (int)luaL_checkinteger(L, 2);
    double pen = 1.0;
    if (lua_isnumber(L, 3)) pen = luaL_checknumber(L, 3);
...
static int shapetool_setmarks(lua_State * L) {
	...
	luaL_argcheck(L, ShapeTool::EVertex <= t && t < ShapeTool::ENumMarkTypes, 1,
		      "number is not a mark type");
	lua_pop(L, 2); // v, t
...
static int appui_setNumbers(lua_State * L) {
    ...
    String vno;
    String pno;
    if (!lua_isnil(L, 2)) vno = luaL_checklstring(L, 2, nullptr);
    bool vm = lua_toboolean(L, 3);
    if (!lua_isnil(L, 4)) pno = luaL_checklstring(L, 4, nullptr);
...
static int appui_showTool(lua_State * L) {
    static const char * const option_names[] = {"properties", "bookmarks", "notes",
						"layers", nullptr};
...
static int appui_explain(lua_State * L) {
    ...
    const char * s = luaL_checklstring(L, 2, nullptr);
    int t = 4000;
    if (lua_isnumber(L, 3)) t = lua_tointegerx(L, 3, nullptr);
    (*ui)->explain(s, t);
    return 0;
}
...
void get_page_sorter_size(lua_State * L, int & width, int & height, int & thumbWidth) {
    thumbWidth = 300;
    width = 600;
    height = 480;
    ...
    if (lua_isnumber(L, -1)) thumbWidth = lua_tointegerx(L, -1, nullptr);
...
static int appui_selectPage(lua_State * L) {
	...
	luaL_argcheck(L, 1 <= page && page <= (*doc)->countPages(), 3,
		      "invalid page number");
    }
	...
	int maxIndex =
	    (page < 0) ? (*doc)->countPages() : (*doc)->page(page - 1)->countViews();
	luaL_argcheck(L, 1 <= startIndex && startIndex <= maxIndex, 4,
		      "invalid start index");
    }
    ...
    int sel = CanvasBase::selectPageOrView(*doc, page - 1, startIndex - 1, thumbWidth,
					   width, height);
    if (sel >= 0) {
...
static int appui_constructor(lua_State * L) {
    ...
    *ui = createAppUi(L, model);
	...
	if (!lua_isnil(L, -1)) style.paperColor = check_color(L, lua_gettop(L));
	...
	if (!lua_isnil(L, -1)) style.gridLineColor = check_color(L, lua_gettop(L));
	lua_pop(L, 1); // grid_line_color

	lua_getfield(L, -1, "path_view_color");
	if (!lua_isnil(L, -1)) pathViewColor = check_color(L, lua_gettop(L));
	...
	if (lua_isnumber(L, -1)) style.thinLine = lua_tonumberx(L, -1, nullptr);
	lua_pop(L, 1); // thin_grid_line

	lua_getfield(L, -1, "thick_grid_line");
	if (lua_isnumber(L, -1)) style.thickLine = lua_tonumberx(L, -1, nullptr);
	lua_pop(L, 1); // thick_grid_line

	lua_getfield(L, -1, "thin_step");
	if (lua_isnumber(L, -1)) style.thinStep = lua_tointegerx(L, -1, nullptr);
	lua_pop(L, 1); // thin_step

	lua_getfield(L, -1, "thick_step");
	if (lua_isnumber(L, -1)) style.thickStep = lua_tointegerx(L, -1, nullptr);
