...
static const char * const snapbutton_action[] = {
    "snapvtx",   "snapctl",    "snapbd",   "snapint",     "snapgrid",
    "snapangle", "snapcustom", "snapauto", "grid_visible"};

static const char * const touchbar_action[] = {"escape",
					       "set_origin",
					       "set_direction",
					       "set_line",
					       "show_axes",
...
static const char * const touchbar_titles[] = {
    "esc",   "org",   "dir",   "line",  "axes",  "reset", "tangent", nullptr,
    nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};

static const char * const touchbar_customization_label[] = {"Escape",
							    "Set origin",
							    "Set direction",
							    "Set to edge",
							    "Show axes",
							    "Reset direction",
							    "Set to tangent direction",
							    "Snap to vertex",
...
- (instancetype)initWithTitle:(NSString *)aTitle
    ...
    self =
	[super initWithTitle:aTitle action:@selector(ipeMenuAction:) keyEquivalent:aKey];
    if (self) {
	...
	// make sure these work when no window exists
		     ...
		     @"show_configuration", @"new_window", @"open", @"manual",
		     @"about_ipelets"
...
@interface IpeWindowDelegate
    : NSWindowController <NSWindowDelegate, NSToolbarDelegate, IpeControlsDelegate,
			  NSTouchBarDelegate>
@end
...
- (NSToolbarItem *)toolbar:(NSToolbar *)toolbar
    ...
    NSToolbarItem * t = [[NSToolbarItem alloc] initWithItemIdentifier:itemIdentifier];
...
- (NSArray *)toolbarSelectableItemIdentifiers:(NSToolbar *)toolbar {
	...
	@"mode_label",       @"mode_math",        @"mode_paragraph",
	@"mode_marks",       @"mode_rectangles1", @"mode_rectangles2",
	@"mode_rectangles2", @"mode_rectangles3", @"mode_parallelogram",
	@"mode_lines",       @"mode_polygons",    @"mode_splines",
	@"mode_splinegons",  @"mode_arc1",        @"mode_arc2",
	@"mode_arc3",        @"mode_circle1",     @"mode_circle2",
	@"mode_circle3",     @"mode_ink"
    ];
}
...
- (NSTouchBar *)makeTouchBar {
	...
	@"show_axes", @"set_origin", @"set_direction", @"snapvtx", @"snapctl", @"snapbd",
	@"snapint", @"snapgrid"
    ];
    bar.customizationIdentifier = @"org.otfried.ipe.mainbar7_2_11";
    bar.customizationAllowedItemIdentifiers = @[
	@"show_axes", @"set_origin", @"set_direction", @"reset_direction", @"set_line",
	@"set_tangent_direction", @"snapvtx", @"snapctl", @"snapbd", @"snapint",
...
- (NSTouchBarItem *)touchBar:(NSTouchBar *)bar
	...
	if (idf == touchbar_action[tag]) {
		...
		b = [NSButton buttonWithTitle:identifier
					image:image
				       target:self
				       action:@selector(ipeTouchBar:)];
		b.buttonType = NSPushOnPushOffButton;
		b.imagePosition = NSImageOnly;
	    } else {
		b = [NSButton buttonWithTitle:C2N(title)
				       target:self
				       action:@selector(ipeTouchBar:)];
		if (idf == "show_axes") b.buttonType = NSPushOnPushOffButton;
	    }
	    b.tag = tag;
	    item.view = b;
...
void AppUi::addItem(NSMenu * menu, const char * title, const char * name) {
    if (title == nullptr) {
	if (build_menus) [menu addItem:[NSMenuItem separatorItem]];
    } else {
	...
	String sc;
	if (lua_isstring(L, -1)) sc = lua_tostring(L, -1);
	...
	NSString * keyEq = @"";
	if (code) keyEq = [NSString stringWithFormat:@"%C", code];

	NSMenuItem * item =
	    [[IpeMenuItem alloc] initWithTitle:ipeui_set_mnemonic(I2N(title), nil)
				     ipeAction:C2N(name)
				 keyEquivalent:keyEq];
...
void AppUi::startSubMenu(int id, const char * name, int tag) {
    ...
    NSMenuItem * item =
	[[NSMenuItem alloc] initWithTitle:title action:nullptr keyEquivalent:@""];
...
MENUHANDLE AppUi::endSubMenu() { return current_submenu; }
...
AppUi::AppUi(lua_State * L0, int model)
    ...
    if (@available(macOS 11, *)) [iWindow setToolbarStyle:NSWindowToolbarStyleExpanded];
#endif
    ...
    [iStatus setContentHuggingPriority:NSLayoutPriorityDefaultLow
			forOrientation:NSLayoutConstraintOrientationHorizontal];
    [iSnapIndicator setContentHuggingPriority:NSLayoutPriorityDefaultHigh
			       forOrientation:NSLayoutConstraintOrientationHorizontal];
    [iMouseIndicator setContentHuggingPriority:NSLayoutPriorityDefaultHigh
				forOrientation:NSLayoutConstraintOrientationHorizontal];
    [iZoomIndicator setContentHuggingPriority:NSLayoutPriorityDefaultHigh
			       forOrientation:NSLayoutConstraintOrientationHorizontal];
...
void AppUi::makePropertiesTool() {
	...
	iSelector[i] = [[NSPopUpButton alloc] initWithFrame:NSMakeRect(0., 0., 50., 20.)
						  pullsDown:NO];
    ...
    iPathView.toolTip = @"Toggle arrows, toggle fill mode, right-click for path style";
...
void AppUi::makeSnapBar() {
	...
	if (s) iSnapButton[i].toolTip = s.title;
	...
	if (0 < i && i < 5) layout(iSnapButton[i], iSnapButton[i - 1], "l=r", PAD);
    }
...
void AppUi::resetCombos() {
    iInUiUpdate = true;
    for (int i = 0; i < EUiView; ++i) [iSelector[i] removeAllItems];
    iInUiUpdate = false;
}
...
void AppUi::setCheckMark(String name, String value) {
	...
	if ([action hasPrefix:prefix]) iActions[action].state = NO;
    }
...
void AppUi::setActionsEnabled(bool mode) { iActionsEnabled = mode; }
...
BOOL AppUi::validateMenuItem(NSMenuItem * item, NSString * name) {
	...
	item.title = [iSnapBar isHidden] ? @"Show Snap Toolbar" : @"Hide Snap Toolbar";
	return YES;
    }
...
void AppUi::fillDynamicSubmenu(NSMenuItem * item) {
	...
	String cmd =
	    (item.tag == ESubmenuSelectLayer) ? "selectinlayer-" : "movetolayer-";
	for (auto & name : iLayerNames) {
	    NSMenuItem * mi = [[IpeMenuItem alloc] initWithTitle:I2N(name)
	...
	for (auto & attr : seq) {
	    NSMenuItem * mi = [[IpeMenuItem alloc] initWithTitle:I2N(attr.string())
						       ipeAction:I2N(cmd + attr.string())
						   keyEquivalent:@""];
	    if (attr == iAll.iTextStyle) mi.state = NSOnState;
	    [sm addItem:mi];
	}
	...
	for (auto & name : iComboContents[uisel]) {
	    NSMenuItem * mi = [[IpeMenuItem alloc] initWithTitle:I2N(name)
						       ipeAction:I2N(cmd + name)
						   keyEquivalent:@""];
	    if (count == curr) mi.state = NSOnState;
	    ++count;
	    [sm addItem:mi];
	}
...
void AppUi::setRecentFileMenu(const std::vector<String> & names) {
    iRecentFiles.clear();
    for (const auto & name : names) iRecentFiles.push_back(name);
}
...
void AppUi::setNotes(String notes) {
	...
	NSAttributedString * n = [[NSAttributedString alloc] initWithString:I2N(notes)];
...
void AppUi::setToolVisible(int m, bool vis) {
	...
	if (vis && iNotesPanel == nil) {
	    ...
	    [iNotesField setAutoresizingMask:NSViewWidthSizable | NSViewHeightSizable];
...
void AppUi::absoluteButton(int sel) { luaAbsoluteButton(selectorNames[sel]); }
...
bool AppUi::actionState(const char * name) {
    if (!strcmp(name, "viewmarked")) return iViewMarked.state == NSOnState;
    if (!strcmp(name, "pagemarked")) return iPageMarked.state == NSOnState;
...
void AppUi::setActionState(const char * name, bool value) {
	...
	if (iCanvas) iCanvas->setAdditionalModifiers(mod);
    }
}
...
void AppUi::action(String name) {
    ...
    if (name == "escape") {
	if (iCanvas && iCanvas->tool()) iCanvas->tool()->key("\033", 0);
	return;
    }
    ...
    if (s && s.toggles) setActionState(name.z(), !s.state);
    luaAction(name);
}
...
void AppUi::setSnapIndicator(const char * s) { [iSnapIndicator setStringValue:C2N(s)]; }

void AppUi::setMouseIndicator(const char * s) { [iMouseIndicator setStringValue:C2N(s)]; }

void AppUi::setZoom(double zoom) {
    iCanvas->setZoom(zoom);
    [iZoomIndicator
	setStringValue:[NSString stringWithFormat:@"%3dppi", int(72.0 * zoom)]];
}
...
void AppUi::closeWindow() {
    dispatch_async(dispatch_get_main_queue(), ^{ [iWindow performClose:iDelegate]; });
}
...
NSImage * AppUi::createIcon(int pno, int size, bool touchBar) {
    ...
    return [NSImage imageWithSize:s
		   ...
		   drawingHandler:^(NSRect rect) {
		     auto col = [[NSColor textColor]
			 colorUsingColorSpace:[NSColorSpace genericGrayColorSpace]];
...
int AppUi::clipboard(lua_State * L) {
    ...
    NSArray * arr = [pasteBoard readObjectsForClasses:@[ [NSString class] ] options:@{}];
    if (arr && [arr count] > 0) {
...
bool AppUi::waitDialog(const char * cmd, const char * label) {
    ...
    NSTextField * l = [[NSTextField alloc] initWithFrame:NSMakeRect(0., 0., 200., 100.)];
