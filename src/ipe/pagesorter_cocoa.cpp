...
- (id)initWithFrame:(NSRect)frameRect {
	...
	NSImageView * v =
	    [[NSImageView alloc] initWithFrame:NSMakeRect(2., 20., thumbnail_size.width,
...
@interface IpePageSorterDelegate : NSObject <NSWindowDelegate, NSCollectionViewDelegate>
...
- (BOOL)collectionView:(NSCollectionView *)cv
    ...
    for (size_t i = 0; i < [move count]; ++i) [nIdx addIndex:dest++];
...
int AppUi::pageSorter(lua_State * L, Document * doc, int pno, int width, int height,
    ...
    [cv bind:NSContentBinding toObject:delegate withKeyPath:@"pages" options:nullptr];
