...
PathView::PathView(HWND parent, int id) {
	...
	WS_EX_CLIENTEDGE, className, L"", WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, parent,
	nullptr, (HINSTANCE)GetWindowLongPtr(parent, GWLP_HINSTANCE), this);
    if (hwnd == nullptr) {
...
LRESULT CALLBACK PathView::wndProc(HWND hwnd, UINT message, WPARAM wParam,
	...
	return 0;
    case WM_LBUTTONDOWN:
	if (pv) pv->button(LOWORD(lParam), HIWORD(lParam));
	break;
    case WM_RBUTTONUP:
	if (pv) {
	...
	break;
    case WM_DESTROY: delete pv; break;
    default: break;
    }
...
void PathView::button(int x, int y) {
	...
	case EStrokedOnly: iAction = "pathmode|strokedfilled"; break;
	case EStrokedAndFilled: iAction = "pathmode|filled"; break;
	case EFilledOnly: iAction = "pathmode|stroked"; break;
	}
	SendMessage(GetParent(hwnd), WM_COMMAND, idBase + 1, (LPARAM)hwnd);
    }
    // show new status
