...
void AppUi::addItem(QMenu * m, const QString & title, const char * name) {
    ...
    bool checkable = (m == iMenu[EModeMenu]) || (String(name).find('|') >= 0);
    if (name[0] == '*') {
	checkable = true;
	name = name + 1;
    }
...
AppUi::AppUi(lua_State * L0, int model, Qt::WindowFlags f)
    ...
    connect(iGridSizeMenu, SIGNAL(triggered(QAction *)), SLOT(gridSizeAction(QAction *)));
    ...
    addDockWidget(getDockSide(L, "properties", Qt::LeftDockWidgetArea), iPropertiesTools);
    iPropertiesTools->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);

    iBookmarkTools = new QDockWidget("Bookmarks", this);
    addDockWidget(getDockSide(L, "bookmarks", Qt::RightDockWidgetArea), iBookmarkTools);
    iBookmarkTools->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    iMenu[EPageMenu]->addAction(iBookmarkTools->toggleViewAction());

    iNotesTools = new QDockWidget("Notes", this);
    addDockWidget(getDockSide(L, "notes", Qt::RightDockWidgetArea), iNotesTools);
    iNotesTools->setAllowedAreas(Qt::LeftDockWidgetArea | Qt::RightDockWidgetArea);
    iMenu[EPageMenu]->addAction(iNotesTools->toggleViewAction());

    iLayerTools = new QDockWidget("Layers", this);
    addDockWidget(getDockSide(L, "layers", Qt::LeftDockWidgetArea), iLayerTools);
	...
	if (i != EUiDashStyle && i != EUiMarkShape && i != EUiOpacity) {
	    ...
	    iButton[i]->setSizePolicy(QSizePolicy(
		QSizePolicy::Fixed, QSizePolicy::Expanding, QSizePolicy::ToolButton));
    ...
    connect(iBookmarks, SIGNAL(itemActivated(QListWidgetItem *)), this,
	    SLOT(bookmarkSelected(QListWidgetItem *)));
    ...
    connect(iLayerList, SIGNAL(activated(String, String)), this,
	    SLOT(layerAction(String, String)));
    connect(iLayerList, SIGNAL(showLayerBoxPopup(Vector, String)),
	    SLOT(showLayerBoxPopup(Vector, String)));

    connect(iSelectLayerMenu, SIGNAL(aboutToShow()), this,
	    SLOT(aboutToShowSelectLayerMenu()));
    connect(iMoveToLayerMenu, SIGNAL(aboutToShow()), this,
	    SLOT(aboutToShowMoveToLayerMenu()));
    connect(iTextStyleMenu, SIGNAL(aboutToShow()), this,
	    SLOT(aboutToShowTextStyleMenu()));
    connect(iLabelStyleMenu, SIGNAL(aboutToShow()), this,
	    SLOT(aboutToShowLabelStyleMenu()));
    connect(iGridSizeMenu, SIGNAL(aboutToShow()), this, SLOT(aboutToShowGridSizeMenu()));
    connect(iAngleSizeMenu, SIGNAL(aboutToShow()), this,
	    SLOT(aboutToShowAngleSizeMenu()));
...
void AppUi::recentFileAction(QAction * a) { luaRecentFileSelected(IpeQ(a->text())); }
...
void AppUi::aboutToShowStyleMenu(Kind kind, MENUHANDLE menu, String current) {
	...
	a->setCheckable(true);
	if (s == current) a->setChecked(true);
	menu->addAction(a);
    }
}
...
void AppUi::aboutToShowGridSizeMenu() {
	...
	a->setCheckable(true);
	if (s == iSelector[EUiGridSize]->currentText()) a->setChecked(true);
...
void AppUi::aboutToShowAngleSizeMenu() {
	...
	a->setCheckable(true);
	if (s == iSelector[EUiAngleSize]->currentText()) a->setChecked(true);
...
void AppUi::toolbarModifiersChanged() {
    if (iCanvas) {
	int mod = 0;
	if (iShiftKey->isChecked()) mod |= CanvasBase::EShift;
	iCanvas->setAdditionalModifiers(mod);
    }
}

void AppUi::abortDrawing() { action("stop"); }

// --------------------------------------------------------------------

void AppUi::resetCombos() {
    for (int i = 0; i < EUiView; ++i) iSelector[i]->clear();
}
...
void AppUi::setCheckMark(String name, Attribute a) {
    ...
    for (std::map<String, QAction *>::iterator it = iActions.begin();
	 it != iActions.end(); ++it) {
	if (it->first.left(na) == sa) it->second->setChecked(it->first == sb);
    }
}
...
void AppUi::setNotes(String notes) { iPageNotes->setPlainText(QIpe(notes)); }
...
void AppUi::setBookmarks(int no, const String * s) {
	...
	if (s[i][0] == ' ') item->setForeground(Qt::blue);
...
static void enableActions(QMenu * menu, bool mode) {
    ...
    while (it.hasNext()) it.next()->setEnabled(mode);
}
...
bool AppUi::actionState(const char * name) {
    if (!strcmp(name, "viewmarked")) return (iViewMarked->checkState() == Qt::Checked);
    if (!strcmp(name, "pagemarked")) return (iPageMarked->checkState() == Qt::Checked);
...
void AppUi::qAction(const QString & name) { action(IpeQ(name)); }
...
void AppUi::action(String name) {
	...
	if (name.left(5) == "mode_") iModeIndicator->setPixmap(prefsPixmap(name));
	luaAction(name);
    }
}
...
void AppUi::showPathStylePopup(Vector v) { luaShowPathStylePopup(v); }

void AppUi::showLayerBoxPopup(Vector v, String layer) { luaShowLayerBoxPopup(v, layer); }
...
void AppUi::setMouseIndicator(const char * s) { iMouse->setText(s); }

void AppUi::setSnapIndicator(const char * s) { iSnapIndicator->setText(s); }
...
void AppUi::setFullScreen(int mode) {
    ...
    switch (mode) {
    case 1: state |= Qt::WindowMaximized; break;
    case 2: state |= Qt::WindowFullScreen; break;
    default: break;
    }
...
int AppUi::clipboard(lua_State * L) {
	...
	if (md->hasUrls()) {
		...
		if (readImage(L, IpeQ(urls[0].toLocalFile()))) return 1;
	    }
	}
	QImage img = cb->image();
	if (!img.isNull()) { return getImage(L, img); }
    }
...
Waiter::Waiter(const QString & cmd)
    : iCommand{cmd} {
    // nothing
}
