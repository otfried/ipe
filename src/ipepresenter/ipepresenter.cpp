  ...
  for (int i = 0; i < iPdf->countPages(); ++i) {
	...
	if (a && a->dict()) {
	  ...
	  if (type && type->name() && type->name()->value() == "Annot") {
	    if (subtype && subtype->name() && subtype->name()->value() == "Text" &&
		contents && contents->string()) {
	      if (!notes.empty())
		notes += "\n";
	      notes += contents->string()->decode();
	    } else if (subtype && subtype->name() && subtype->name()->value() == "Link") {
	      std::vector<double> drect;
	      Rect rect;
	      if (d->getNumberArray("Rect", iPdf.get(), drect) && drect.size() == 4) {
		rect.addPoint(Vector(drect[0], drect[1]));
		rect.addPoint(Vector(drect[2], drect[3]));
	      }
	      const PdfDict *action = d->getDict("A", iPdf.get());
	      if (action)
		links.push_back({rect, action});
	    }
	  }
	}
      }
...
void Presenter::collectDestinations(const PdfDict *d)
{
  ...
  if (kids) {  // recurse
	...
	if (value->dict())
	  value = value->dict()->get("D", iPdf.get());
	if (value->array()) {
...
void Presenter::interpretAction(const PdfDict *action)
{
  ...
  if (type == "URI") {
    ...
    if (uriObj && uriObj->string())
      browseLaunch(false, uriObj->string()->value());
  } else if (type == "Launch") {
    const PdfObj *fObj = action->get("F", iPdf.get());
    if (fObj && fObj->string())
      browseLaunch(true, fObj->string()->value());
  } else if (type == "GoTo") {
