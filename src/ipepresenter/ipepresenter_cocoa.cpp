...
@interface AppDelegate
    : NSObject <NSApplicationDelegate, NSWindowDelegate, NSSplitViewDelegate>
@end
...
void AppUi::setView() {
    ...
    setViewPage(iNext.pdfView,
		iPdfPageNo < iPdf->countPages() - 1 ? iPdfPageNo + 1 : iPdfPageNo);

    [iWindow setTitle:I2N(currentLabel())];
    NSAttributedString * n =
	[[NSAttributedString alloc] initWithString:I2N(iAnnotations[iPdfPageNo])];
...
void AppUi::setTime() {
    [iClock setStringValue:[NSString stringWithFormat:@"%d:%02d:%02d", iTime / 3600,
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    if (ui->iFileName.empty()) [self openDocument:self];
}
...
- (void)openDocument:(id)sender {
    ...
    [panel beginSheetModalForWindow:ui->iWindow
		  completionHandler:^(NSInteger result) {
		    if (result == NSFileHandlingPanelOKButton) {
			NSURL * url = [[panel URLs] objectAtIndex:0];
			if ([url isFileURL]) ui->load([url path]);
		    }
		    // canceled or failed to load and we didn't have a document before
		    if (ui->iFileName.empty()) [NSApp terminate:self];
		  }];
}
...
- (void)aboutIpePresenter:(id)sender {
	...
	stringWithFormat:@(about_text), ipe::IPELIB_VERSION / 10000,
