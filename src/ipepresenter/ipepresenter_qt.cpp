...
IpeAction::IpeAction(int cmd, const QString & text, const char * shortcut,
    ...
    : QAction(text, parent)
    , iCommand{cmd} {
    if (shortcut) setShortcut(QKeySequence(shortcut));
    connect(this, &QAction::triggered, [parent, this]() { parent->cmd(iCommand); });
}

// --------------------------------------------------------------------

BeamerView::BeamerView(Qt::WindowFlags f)
...
MainWindow::MainWindow(BeamerView * bv, Qt::WindowFlags f)
    ...
    iShowPresentationAction =
	new IpeAction(EShowPresentation, "Show presentation", "F5", this);
    ...
    connect(iViewMenu, &QMenu::aboutToShow, [this]() {
	iShowPresentationAction->setChecked(iScreen->isVisible());
	iFullScreenAction->setChecked((iScreen->windowState() & Qt::WindowFullScreen)
				      != 0);
    ...
    QList<QKeySequence> nextKeys{
	QKeySequence("Right"),
	QKeySequence("Down"),
	QKeySequence("PgDown"),
    };
    QList<QKeySequence> prevKeys{QKeySequence("Left"), QKeySequence("Up"),
				 QKeySequence("PgUp")};
    ...
    connect(iNext, &PdfView::sizeChanged, [this]() { fitBox(mediaBox(-2), iNext); });

    connect(iCurrent, &PdfView::mouseButton, [this](int button, const Vector & pos) {
...
void MainWindow::cmd(int c) {
	...
	break;
	//
    case EToggleTimeCounting: iClock->toggleCounting(); break;
    case ETimeCountdown: iClock->toggleCountdown(); break;
    case ESetTime: iClock->setTime(); break;
    case EResetTime:
	iClock->resetTime();
	break;
	//
...
void MainWindow::selectPage() {
	...
	for (int i = 0; i < iPdf->countPages(); ++i) {
	    ...
	    QImage bits((const uchar *)b.data(), r.width(), r.height(),
			QImage::Format_RGB32);
