...
extern int showPageSelectDialog(int width, int height, const char * title, HIMAGELIST il,
				std::vector<String> & labels, int startIndex);
...
void AppUi::initUi() {
    ...
    */
    ...
    hClock = CreateWindowExW(0, L"static", nullptr,
			     WS_CHILD | WS_VISIBLE | WS_BORDER | SS_CENTER, 0, 0, 0, 0,
			     hwnd, nullptr, hInstance, nullptr);

    hFont = CreateFontW(48, 0, 0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, ANSI_CHARSET,
			OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
			DEFAULT_PITCH | FF_SWISS, L"MS Shell Dlg");

    if (hFont != nullptr) SendMessage(hClock, WM_SETFONT, WPARAM(hFont), TRUE);

    SetTimer(hwnd, 1, 1000, nullptr);
}

AppUi::~AppUi() {
    if (iThumbNails) ImageList_Destroy(iThumbNails);
    KillTimer(hwnd, 1);
    delete iScreen;
...
void AppUi::layoutChildren() {
    if (iCurrent == nullptr) return; // no children yet
    ...
    MoveWindow(hNotes, splitH + 10, splitClock + 10, wid, splitV - splitClock - 10, TRUE);
...
void AppUi::handlePdfViewMessage(int param, const Vector & pos) {
    ...
    switch (param & 0x0f) {
    case 0: fitBoxAll(); break;
    case 1: cmd(ENextView); break;
    case 2: cmd(EPreviousView); break;
    }
}
...
void AppUi::setTime() {
    char buf[16];
    sprintf(buf, "%d:%02d:%02d", iTime / 3600, (iTime / 60) % 60, iTime % 60);
...
void AppUi::zoomMain(int delta) {
    ...
    if (50 <= nPerc && nPerc <= 80) iMainPercentage = nPerc;
    layoutChildren();
}
...
void AppUi::cmd(int cmd) {
	...
	break;
    case EFullScreen: toggleFullScreen(); break;
    case EBlackout:
	iScreen->setBlackout(!iScreen->blackout());
	iScreen->updatePdf();
	break;
    case EZoomIn: zoomMain(+1); break;
    case EZoomOut:
	zoomMain(-1);
	break;
	//
	...
	CheckMenuItem(hMenuBar, EToggleTimeCounting,
		      iCountTime ? MF_CHECKED : MF_UNCHECKED);
	break;
    case ETimeCountdown:
	...
	setView();
	break;
    case EJumpTo: jumpTo(); break;
    case ESelectPage: selectPage(); break;
    case EAbout: aboutIpePresenter(); break;
    default:
	// unknown action
	return;
    }
...
static const char * const aboutText =
    ...
    "IpePresenter is now developed together with Ipe and released under the GNU Public "
    "License.\n"
    ...
    " * SCALGO\n"
    " * Martin Ziegler\n\n"
    "If you enjoy IpePresenter, feel free to treat the author on a cup of coffee at "
...
void AppUi::setTimerValue() {
    ...
    if (DialogBox(hInstance, MAKEINTRESOURCE(IDD_INPUTBOX), hwnd, (DLGPROC)SetTimeProc)
	== IDOK) {
	Lex lex(dialogInput);
...
void AppUi::selectPage() {
    ...
    for (int i = 0; i < iPdf->countPages(); ++i) labels.push_back(pageLabel(i));
...
LRESULT CALLBACK AppUi::wndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (ui && lParam == 0) {
	    CheckMenuItem(ui->hMenuBar, EShowPresentation,
			  IsWindowVisible(ui->iScreen->windowId()) ? MF_CHECKED
								   : MF_UNCHECKED);
	    CheckMenuItem(ui->hMenuBar, EFullScreen,
			  ui->iFullScreen ? MF_CHECKED : MF_UNCHECKED);
	}
	break;
    case WM_COMMAND:
	...
	break;
    case WM_SIZE:
	if (ui) ui->layoutChildren();
	break;
    case WM_TIMER:
	if (ui) ui->timerElapsed();
	break;
...
static String getFileName() {
    ...
    if (GetOpenFileNameW(&ofn)) return String(szFileName);
    return String();
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine,
    ...
    if (fname.empty()) fname = getFileName();
