...
Reference::Reference(const AllAttributes & attr, Attribute name, Vector pos)
    ...
    if (iFlags & EHasPen) iPen = attr.iPen;
    if (iFlags & EHasSize) iSize = attr.iSymbolSize;
    if (iFlags & EHasStroke) iStroke = attr.iStroke;
    if (iFlags & EHasFill) iFill = attr.iFill;
}
...
//! Clone object
Object * Reference::clone() const { return new Reference(*this); }

//! Return pointer to this object.
Reference * Reference::asReference() { return this; }

Object::Type Reference::type() const { return EReference; }

//! Call visitReference of visitor.
void Reference::accept(Visitor & visitor) const { visitor.visitReference(this); }
...
void Reference::saveAsXml(Stream & stream, String layer) const {
    ...
    if (iPos != Vector::ZERO) stream << " pos=\"" << iPos << "\"";
...
void Reference::draw(Painter & painter) const {
	...
	painter.push();
	if (iFlags & EHasStroke) painter.setSymStroke(iStroke);
	if (iFlags & EHasFill) painter.setSymFill(iFill);
	if (iFlags & EHasPen) painter.setSymPen(iPen);
...
void Reference::addToBBox(Rect & box, const Matrix & m, bool cp) const {
    if (iSnap.size() > 0) {
	for (const Vector & pos : iSnap) box.addPoint((m * matrix()) * (iPos + pos));
    } else
...
void Reference::checkStyle(const Cascade * sheet, AttributeSeq & seq) const {
    ...
    if (!symbol) {
	if (std::find(seq.begin(), seq.end(), iName) == seq.end()) seq.push_back(iName);
    } else {
	Attribute si = sheet->find(ESymbolSize, iSize);
	double s = si.number().toDouble();
	cacheSnaps(symbol, (iFlags & EHasSize) ? s : 1.0);
    }
    if (iFlags & EHasStroke) checkSymbol(EColor, iStroke, sheet, seq);
    if (iFlags & EHasFill) checkSymbol(EColor, iFill, sheet, seq);
    if (iFlags & EHasPen) checkSymbol(EPen, iPen, sheet, seq);
    if (iFlags & EHasSize) checkSymbol(ESymbolSize, iSize, sheet, seq);
}
...
void Reference::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos,
			double & bound) const {
    if (iSnap.size() > 0) {
	for (const Vector & snapPos : iSnap)
...
void Reference::snapBnd(const Vector &, const Matrix &, Vector &, double &) const {
    // nothing
}
...
//! Set pen.
void Reference::setPen(Attribute pen) { iPen = pen; }

//! Set stroke color.
void Reference::setStroke(Attribute color) { iStroke = color; }

//! Set fill color.
void Reference::setFill(Attribute color) { iFill = color; }

//! Set size (magnification) of symbol.
void Reference::setSize(Attribute size) { iSize = size; }
...
Attribute Reference::getAttribute(Property prop) const noexcept {
    switch (prop) {
    case EPropPen:
	if (iFlags & EHasPen) return pen();
	break;
    case EPropStrokeColor:
	if (iFlags & EHasStroke) return stroke();
	break;
    case EPropFillColor:
	if (iFlags & EHasFill) return fill();
	break;
    case EPropSymbolSize:
	if (iFlags & EHasSize) return size();
	break;
    case EPropMarkShape:
	if (iFlags & EIsMark) return name();
	break;
    default: break;
    }
...
void Reference::cacheSnaps(const Symbol * symbol, double size) const {
    iSnap.clear();
    for (const Vector & v : symbol->iSnap) iSnap.push_back(size * v);
}
...
uint32_t Reference::flagsFromName(String name) {
    uint32_t flags = 0;
    if (name.left(5) == "mark/") flags |= EIsMark;
    if (name.left(6) == "arrow/") flags |= EIsArrow;
    int i = name.rfind('(');
    if (i < 0 || name[name.size() - 1] != ')') return flags;
    String letters = name.substr(i + 1, name.size() - i - 2);
    if (letters.find('x') >= 0) flags |= EHasSize;
    if (letters.find('s') >= 0) flags |= EHasStroke;
    if (letters.find('f') >= 0) flags |= EHasFill;
    if (letters.find('p') >= 0) flags |= EHasPen;
    return flags;
}
