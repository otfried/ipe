...
void Page::saveAsXml(Stream & stream) const {
	...
	if (iLayers[i].locked) stream << " edit=\"no\"";
	...
	stream << "\"";
	if (!active(i).empty()) stream << " active=\"" << active(i) << "\"";
	if (!effect(i).isNormal()) stream << " effect=\"" << effect(i).string() << "\"";
	if (markedView(i)) stream << " marked=\"yes\"";
	if (!viewName(i).empty()) stream << " name=\"" << viewName(i) << "\"";
	...
	if (map.count() == 0 && iViews[i].iLayerMatrices.empty()) {
		...
		stream << "<transform layer=\"" << s.iLayer << "\" matrix=\"" << s.iMatrix
		       << "\"/>\n";
	    }
	    stream << "</view>\n";
	}
    ...
    int currentLayer = -1;
    for (ObjSeq::const_iterator it = iObjects.begin(); it != iObjects.end(); ++it) {
	String l;
...
void Page::setLayerData(int index, String data) { iLayers[index].iData = data; }

//! Set locking of layer \a i.
void Page::setLocked(int i, bool flag) { iLayers[i].locked = flag; }

//! Set snapping of layer \a i.
void Page::setSnapping(int i, SnapMode mode) { iLayers[i].snapMode = mode; }

//! Add a new layer.
void Page::addLayer(String name) {
    iLayers.push_back(SLayer(name));
    iLayers.back().iVisible.resize(countViews());
    for (int i = 0; i < countViews(); ++i) iLayers.back().iVisible[i] = false;
}
...
void Page::moveLayer(int index, int newIndex) {
    assert(0 <= index && index < int(iLayers.size()) && 0 <= newIndex
	   && newIndex < int(iLayers.size()));
...
void Page::objectsPerLayer(std::vector<int> & objcounts) const {
    objcounts.clear();
    for (int i = 0; i < countLayers(); ++i) objcounts.push_back(0);
    for (const auto & obj : iObjects) objcounts[obj.iLayer] += 1;
}
...
Rect Page::pageBBox(const Cascade * sheet) const {
	...
	if (layer(l) == "BBOX")
	    ...
	    for (int vno = 0; !vis && vno < countViews(); ++vno) vis = visible(vno, l);
	layers.push_back(vis);
    }

    BBoxPainter bboxPainter(sheet);
    for (int i = 0; i < count(); ++i) {
	if (layers[layerOf(i)]) object(i)->draw(bboxPainter);
    }
...
Rect Page::viewBBox(const Cascade * sheet, int view) const {
    BBoxPainter bboxPainter(sheet);
    for (int i = 0; i < count(); ++i) {
	if (objectVisible(view, i)) object(i)->draw(bboxPainter);
    }
    return bboxPainter.bbox();
}
...
bool Page::objSnapsInView(int objNo, int view) const noexcept {
    ...
    case SnapMode::Visible: return visible(view, layer);
    case SnapMode::Always: return true;
    case SnapMode::Never:
...
void Page::clearViews() {
    iViews.clear();
    for (LayerSeq::iterator it = iLayers.begin(); it != iLayers.end(); ++it)
	it->iVisible.clear();
}

void Page::setMarkedView(int index, bool marked) { iViews[index].iMarked = marked; }

int Page::countMarkedViews() const {
    int count = 0;
    for (int i = 0; i < countViews(); ++i) {
	if (markedView(i)) ++count;
    }
...
int Page::findView(String s) const {
    ...
    if ('0' <= s[0] && s[0] <= '9') {
	int no = Lex(s).getInt();
	if (no <= 0 || no > countViews()) return -1;
	return no - 1;
    }
    for (int i = 0; i < countViews(); ++i) {
	if (s == viewName(i)) return i;
    }
    return -1;
}
...
std::vector<Matrix> Page::layerMatrices(int view) const {
	...
	if (l >= 0) m[l] = iViews[view].iLayerMatrices[i].iMatrix;
    }
    return m;
}
...
void Page::setLayerMatrix(int view, int layer, const Matrix & m) {
	...
	std::find_if(ms.begin(), ms.end(),
		     [&name](const SLayerMatrix & a) { return a.iLayer == name; });
    if (m.isIdentity()) {
...
const AttributeMap Page::viewMap(int index, const Cascade * sheet) const {
	...
	std::copy(iViews[index].iAttributeMap.iMap.begin(),
		  iViews[index].iAttributeMap.iMap.end(), std::back_inserter(m.iMap));
	return m;
    } else
...
Page::SObject::~SObject() { delete iObject; }
...
void Page::remove(int i) { iObjects.erase(iObjects.begin() + i); }
...
double Page::distance(int i, const Vector & v, double bound) const {
    if (bbox(i).certainClearance(v, bound)) return bound;
...
void Page::invalidateBBox(int i) const { iObjects[i].iBBox.clear(); }
...
void Page::snapVtx(int i, const Vector & mouse, Vector & pos, double & bound) const {
    if (bbox(i).certainClearance(mouse, bound)) return;
...
/*! Looks only for positions closer than \a bound.
  If successful, modifies \a pos and \a bound. */
void Page::snapCtl(int i, const Vector & mouse, Vector & pos, double & bound) const {
    if (bbox(i).certainClearance(mouse, bound)) return;
...
/*! Looks only for positions closer than \a bound.
  If successful, modifies \a pos and \a bound. */
void Page::snapBnd(int i, const Vector & mouse, Vector & pos, double & bound) const {
    if (bbox(i).certainClearance(mouse, bound)) return;
...
void Page::setMarked(bool marked) { iMarked = marked; }
...
Attribute Page::backgroundSymbol(const Cascade * sheet) const {
    if (const auto * ps = sheet->findPageStyle(iStyle)) return ps->iBackground;
...
int Page::primarySelection() const {
    for (int i = 0; i < count(); ++i)
	if (select(i) == EPrimarySelected) return i;
    return -1;
}
...
void Page::deselectAll() {
    for (int i = 0; i < count(); ++i) setSelect(i, ENotSelected);
}
...
void Page::ensurePrimarySelection() {
    for (int i = 0; i < count(); ++i)
	if (select(i) == EPrimarySelected) return;
...
void Page::saveSelection(Stream & stream) const {
    BitmapFinder bmFinder;
    for (int i = 0; i < count(); ++i) {
	if (select(i)) object(i)->accept(bmFinder);
    }
    stream << "<ipeselection>\n";
    int id = 1;
    for (std::vector<Bitmap>::const_iterator it = bmFinder.iBitmaps.begin();
    ...
    for (int i = 0; i < count(); ++i) {
	if (select(i)) object(i)->saveAsXml(stream, layer(layerOf(i)));
    }
    stream << "</ipeselection>\n";
}
