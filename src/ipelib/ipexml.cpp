...
XmlAttributes::XmlAttributes() { iSlash = false; }
...
//! Add a new attribute.
void XmlAttributes::add(String key, String val) { iMap[key] = val; }
...
bool XmlAttributes::has(String str) const { return (iMap.find(str) != iMap.end()); }
...
String XmlParser::parseToTagX() {
	...
	if (comment_found) {
		...
		// skip to end of tag
		while (!eos() && iCh != '>') getChar();
	    }
	    getChar();
	    if (eos()) return String();
	}
    } while (comment_found);
    String tagname;
    if (iCh == '?' || iCh == '/') {
...
String XmlParser::parseToTag() {
	...
	if (s.size() < 3 || (s[0] != '/' && (s[0] != 'x' || s[1] != '-'))
	    || (s[0] == '/' && (s[1] != 'x' || s[2] != '-')))
	    return s;
	if (s[0] != '/') {
	    XmlAttributes attr;
	    if (!parseAttributes(attr)) return String();
	}
    }
}
...
bool XmlParser::parseAttributes(XmlAttributes & attr, bool qm) {
	...
	skipWhitespace();
	if (attname.empty() || iCh != '=') return false;
	getChar();
	skipWhitespace();
	// XML allows double or single quotes
	int quote = iCh;
	if (iCh != '\"' && iCh != '\'') return false;
	getChar();
	String val;
	while (!eos() && iCh != quote) {
	    val += char(iCh);
