...
//! Clone object
Object * Image::clone() const { return new Image(*this); }

//! Return pointer to this object.
Image * Image::asImage() { return this; }

Object::Type Image::type() const { return EImage; }

//! Call VisitImage of visitor.
void Image::accept(Visitor & visitor) const { visitor.visitImage(this); }
...
void Image::draw(Painter & painter) const {
    Matrix m(iRect.width(), 0, 0, iRect.height(), iRect.bottomLeft().x,
	     iRect.bottomLeft().y);
    painter.pushMatrix();
...
double Image::distance(const Vector & v, const Matrix & m, double bound) const {
    ...
    u[4] = u[0];
    Rect box;
    for (int i = 0; i < 4; ++i) box.addPoint(u[i]);
...
void Image::snapCtl(const Vector & mouse, const Matrix & m, Vector & pos,
		    double & bound) const {
    Matrix m1 = effectiveMatrix(m);
...
void Image::setOpacity(Attribute opaq) { iOpacity = opaq; }
