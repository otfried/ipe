...
//! Create text object.
Text::Text(const AllAttributes & attr, String data, const Vector & pos, TextType type,
	   double width)
    : Object(attr) {
    iXForm = nullptr;
    iText = data;
...
//! Destructor.
Text::~Text() {
    if (iXForm && --iXForm->iRefCount == 0) delete iXForm;
}
...
Text::Text(const XmlAttributes & attr, String data)
	...
	if (str == "minipage") iType = EMinipage;
    } else if (attr.has("width", str))
	iType = EMinipage; // no type attribute

    if (attr.has("width", str)) iWidth = Lex(str).getDouble();

    iHeight = 10.0;
    if (attr.has("height", str)) iHeight = Lex(str).getDouble();

    iDepth = 0.0;
    if (attr.has("depth", str)) iDepth = Lex(str).getDouble();
    ...
    if (iType == ELabel && iStyle == Attribute::NORMAL() && iText.size() >= 3
	&& iText[0] == '$' && iText[iText.size() - 1] == '$') {
...
void Text::accept(Visitor & visitor) const { visitor.visitText(this); }
...
void Text::saveAsXml(Stream & stream, String layer) const {
    ...
    switch (iType) {
    case ELabel: stream << " type=\"label\""; break;
    case EMinipage: stream << " type=\"minipage\""; break;
    }
...
void Text::saveAlignment(Stream & stream, THorizontalAlignment h, TVerticalAlignment v) {
    switch (h) {
    case EAlignLeft: break;
    case EAlignHCenter: stream << " halign=\"center\""; break;
    case EAlignRight: stream << " halign=\"right\""; break;
    }
    switch (v) {
    case EAlignTop: stream << " valign=\"top\""; break;
    case EAlignBottom: stream << " valign=\"bottom\""; break;
    case EAlignBaseline: stream << " valign=\"baseline\""; break;
    case EAlignVCenter: stream << " valign=\"center\""; break;
    }
}
...
double Text::distance(const Vector & v, const Matrix & m, double bound) const {
    Vector u[5];
    quadrilateral(m, u);
    u[4] = u[0];
    ...
    double d1;
    for (int i = 0; i < 4; ++i) {
	if ((d1 = Segment(u[i], u[i + 1]).distance(v, d)) < d) d = d1;
    }
    return d1;
}
...
void Text::addToBBox(Rect & box, const Matrix & m, bool) const {
    ...
    for (int i = 0; i < 4; ++i) box.addPoint(v[i]);
}

void Text::snapCtl(const Vector & mouse, const Matrix & m, Vector & pos,
		   double & bound) const {
    (m * (matrix() * iPos)).snap(mouse, pos, bound);
    Vector v[4];
    quadrilateral(m, v);
    for (int i = 0; i < 4; ++i) v[i].snap(mouse, pos, bound);
}

// --------------------------------------------------------------------

//! Set stroke color
void Text::setStroke(Attribute stroke) { iStroke = stroke; }

//! Set opacity of the object.
void Text::setOpacity(Attribute opaq) { iOpacity = opaq; }
...
void Text::setVerticalAlignment(TVerticalAlignment align) { iVerticalAlignment = align; }
...
bool Text::setAttribute(Property prop, Attribute value) {
	...
	if ((isMinipage() != (prop == EPropTextStyle)) || value == style()) return false;
	setStyle(value);
	return true;
    case EPropOpacity:
	if (value != opacity()) {
...
Attribute Text::getAttribute(Property prop) const noexcept {
    ...
    case EPropTextSize: return size();
    case EPropTextStyle:
    case EPropLabelStyle: return style();
    case EPropOpacity: return opacity();
    case EPropHorizontalAlignment: return Attribute(horizontalAlignment());
    case EPropVerticalAlignment: return Attribute(verticalAlignment());
    case EPropMinipage: return Attribute::Boolean(isMinipage());
    case EPropWidth: return Attribute(Fixed::fromDouble(width()));
...
void Text::setXForm(XForm * xform) const {
	...
	if (!isMinipage()) iWidth = iXForm->iStretch * iXForm->iBBox.width();
    }
}
...
Vector Text::align() const {
    ...
    case EAlignTop: align.y = totalHeight(); break;
    case EAlignBottom: break;
    case EAlignVCenter: align.y = 0.5 * totalHeight(); break;
    case EAlignBaseline: align.y = depth(); break;
    }
    switch (horizontalAlignment()) {
    case EAlignLeft: break;
    case EAlignRight: align.x = width(); break;
    case EAlignHCenter: align.x = 0.5 * width(); break;
    }
    return align;
}
