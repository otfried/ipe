...
Group::Group(const XmlAttributes & attr)
	...
	Shape clip;
	if (clip.load(str) && clip.countSubPaths() > 0) iClip = clip;
    }
    iUrl = attr["url"];
    iDecoration =
	attr.has("decoration", str) ? Attribute(true, str) : Attribute::NORMAL();
}
...
Group::~Group() {
    if (iImp->iRefCount == 1) {
	for (List::iterator it = iImp->iObjects.begin(); it != iImp->iObjects.end();
	     ++it) {
	    delete *it;
	    *it = nullptr;
	}
...
Object * Group::clone() const { return new Group(*this); }

//! Return pointer to this object.
Group * Group::asGroup() { return this; }

//! Return pointer to this object.
const Group * Group::asGroup() const { return this; }

Object::Type Group::type() const { return EGroup; }
...
void Group::accept(Visitor & visitor) const { visitor.visitGroup(this); }
...
DecorationPainter::DecorationPainter(Painter & painter, const Vector & center, double dx,
				     double dy)
    : Painter(painter.cascade())
    , iPainter(painter)
    , iCenter(center)
    , iX(dx)
    , iY(dy) {
...
void DecorationPainter::doPush() { iPainter.push(); }

void DecorationPainter::doPop() { iPainter.pop(); }
...
void DecorationPainter::doMoveTo(const Vector & v) { iPainter.moveTo(adapt(v)); }

void DecorationPainter::doLineTo(const Vector & v) { iPainter.lineTo(adapt(v)); }
...
void DecorationPainter::doClosePath() { iPainter.closePath(); }

void DecorationPainter::doDrawPath(TPathMode mode) { iPainter.drawPath(mode); }
...
void Group::draw(Painter & painter) const {
    ...
    for (const_iterator it = begin(); it != end(); ++it) (*it)->draw(painter);
    if (iClip.countSubPaths()) painter.pop();
    painter.popMatrix();
}
...
void Group::drawSimple(Painter & painter) const {
    ...
    for (const_iterator it = begin(); it != end(); ++it) (*it)->drawSimple(painter);
    if (iClip.countSubPaths()) painter.pop();
    painter.popMatrix();
}
...
TPinned Group::pinned() const { return TPinned(Object::pinned() | iImp->iPinned); }
...
double Group::distance(const Vector & v, const Matrix & m, double bound) const {
    ...
    Matrix m1 = effectiveMatrix(m);
    for (const_iterator it = begin(); it != end(); ++it) {
	if ((d1 = (*it)->distance(v, m1, d)) < d) d = d1;
    }
    return d;
}

void Group::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos,
		    double & bound) const {
    Matrix m1 = effectiveMatrix(m);
    for (const_iterator it = begin(); it != end(); ++it)
...
void Group::snapCtl(const Vector & mouse, const Matrix & m, Vector & pos,
		    double & bound) const {
    Matrix m1 = effectiveMatrix(m);
    for (const_iterator it = begin(); it != end(); ++it)
...
void Group::snapBnd(const Vector & mouse, const Matrix & m, Vector & pos,
		    double & bound) const {
    Matrix m1 = effectiveMatrix(m);
    for (const_iterator it = begin(); it != end(); ++it)
...
void Group::checkStyle(const Cascade * sheet, AttributeSeq & seq) const {
    for (const_iterator it = begin(); it != end(); ++it) (*it)->checkStyle(sheet, seq);
}

//! Set clip path for this group.
/*! Any previously set clip path is deleted. */
void Group::setClip(const Shape & clip) { iClip = clip; }

//! Set link destination to use this group as a hyperlink.
void Group::setUrl(String url) { iUrl = url; }
...
void Group::detach() {
    ...
    for (const_iterator it = old->iObjects.begin(); it != old->iObjects.end(); ++it)
...
bool Group::setAttribute(Property prop, Attribute value) {
    ...
    detach();
    bool result = false;
    for (List::iterator it = iImp->iObjects.begin(); it != iImp->iObjects.end(); ++it)
