...
Path * Path::create(const XmlAttributes & attr, String data) {
    ...
    if (!self->iShape.load(data)) return nullptr;
    self->makeArrowData();
    return self.release();
}
...
Path::Path(const XmlAttributes & attr)
    ...
    iPathMode = (stroked ? (filled ? EStrokedAndFilled : EStrokedOnly) : EFilledOnly);

    iLineCap = EDefaultCap;
    iLineJoin = EDefaultJoin;
    iFillRule = EDefaultRule;
    if (attr.has("cap", str)) iLineCap = TLineCap(Lex(str).getInt() + 1);
    if (attr.has("join", str)) iLineJoin = TLineJoin(Lex(str).getInt() + 1);
...
Object * Path::clone() const { return new Path(*this); }

//! Compute the arrow information.
void Path::makeArrowData() {
    if (iShape.countSubPaths() == 0 || iShape.countSubPaths() > 1
	|| iShape.subPath(0)->closed()) {
	iFArrowOk = false;
	iRArrowOk = false;
	iMArrowOk = false;
    } else {
	...
	for (int i = 0; i < curve->countSegments(); ++i) {
	    seg = curve->segment(i);
	    if (seg.type() != CurveSegment::ESegment) return;
...
Path * Path::asPath() { return this; }

Object::Type Path::type() const { return EPath; }

//! Call visitPath of visitor.
void Path::accept(Visitor & visitor) const { visitor.visitPath(this); }
...
void Path::saveAsXml(Stream & stream, String layer) const {
    ...
    if (stroked) stream << " stroke=\"" << iStroke.string() << "\"";
    if (filled) stream << " fill=\"" << iFill.string() << "\"";
    ...
    if (stroked && iLineCap != EDefaultCap) stream << " cap=\"" << iLineCap - 1 << "\"";
	...
	stream << " arrow=\"" << s.substr(6, s.size() - 11) << "/" << iFArrowSize.string()
	       << "\"";
    }
    ...
    if (filled && !iTiling.isNormal()) stream << " tiling=\"" << iTiling.string() << "\"";
...
void Path::drawArrow(Painter & painter, Vector pos, Angle angle, Attribute shape,
	...
	if (cw) radius = -radius;
	bool pointy =
	    (shape == Attribute::ARROW_PTARC() || shape == Attribute::ARROW_FPTARC());
	...
	if (shape.isArcArrow() && (radius > s)) {
	    ...
	    Arc arc3;
	    if (cw) {
		arc1 = Arc(Matrix(radius, 0, 0, radius, 0, -radius), IpeHalfPi,
			   IpeHalfPi + delta);
		arc2 = Arc(Matrix(radius, 0, 0, -radius, 0, -radius), -IpeHalfPi - delta,
			   -IpeHalfPi);
		arc3 = Arc(Matrix(radius, 0, 0, radius, 0, -radius), IpeHalfPi,
			   IpeHalfPi + 0.8 * delta);
	    } else {
		arc1 = Arc(Matrix(radius, 0, 0, radius, 0, radius), -IpeHalfPi - delta,
			   -IpeHalfPi);
		arc2 = Arc(Matrix(radius, 0, 0, -radius, 0, radius), IpeHalfPi,
			   IpeHalfPi + delta);
	    ...
	    if (shape == Attribute::ARROW_FARC() || shape == Attribute::ARROW_FPTARC())
	    ...
	    if (cw) {
		if (pointy) painter.lineTo(arc3.endp());
		painter.lineTo(arc2.beginp());
		painter.drawArc(arc2);
	    } else {
		painter.drawArc(arc2);
		if (pointy) painter.lineTo(arc3.beginp());
	    }
	    painter.closePath();
...
void Path::draw(Painter & painter) const {
	...
	if (iHasRArrow && iRArrowOk) {
		...
		drawArrow(painter, iMArrowPos, iMArrowDir + IPE_PI, iRArrowShape,
			  iRArrowSize, r);
	    else
...
void Path::snapVtx(const Vector & mouse, const Matrix & m, Vector & pos,
		   double & bound) const {
    iShape.snapVtx(mouse, effectiveMatrix(m), pos, bound, false);
}

void Path::snapCtl(const Vector & mouse, const Matrix & m, Vector & pos,
		   double & bound) const {
    iShape.snapVtx(mouse, effectiveMatrix(m), pos, bound, true);
}

void Path::snapBnd(const Vector & mouse, const Matrix & m, Vector & pos,
		   double & bound) const {
...
void Path::setPathMode(TPathMode pm) { iPathMode = pm; }

//! Set stroke color.
void Path::setStroke(Attribute stroke) { iStroke = stroke; }

//! Set fill color.
void Path::setFill(Attribute fill) { iFill = fill; }
...
void Path::setOpacity(Attribute opaq) { iOpacity = opaq; }

//! Set stroke opacity of the object.
void Path::setStrokeOpacity(Attribute opaq) { iStrokeOpacity = opaq; }

//! Set pen.
void Path::setPen(Attribute pen) { iPen = pen; }

//! Set dash style.
void Path::setDashStyle(Attribute dash) { iDashStyle = dash; }
...
//! Set line cap style.
void Path::setLineCap(TLineCap s) { iLineCap = s; }

//! Set line join style.
void Path::setLineJoin(TLineJoin s) { iLineJoin = s; }

//! Set fill rule.
void Path::setFillRule(TFillRule s) { iFillRule = s; }
...
void Path::checkStyle(const Cascade * sheet, AttributeSeq & seq) const {
    ...
    if (!iTiling.isNormal()) checkSymbol(ETiling, iTiling, sheet, seq);
    if (!iGradient.isNormal()) checkSymbol(EGradient, iGradient, sheet, seq);
}
...
bool Path::setAttribute(Property prop, Attribute value) {
	...
	break;
    default: return Object::setAttribute(prop, value);
    }
    return false;
}
...
Attribute Path::getAttribute(Property prop) const noexcept {
    switch (prop) {
    case EPropPathMode: return Attribute(iPathMode);
    case EPropStrokeColor: return stroke();
    case EPropFillColor: return fill();
    case EPropPen: return pen();
    case EPropDashStyle: return dashStyle();
    case EPropOpacity: return opacity();
    case EPropStrokeOpacity: return strokeOpacity();
    case EPropTiling: return tiling();
    case EPropGradient: return gradient();
    case EPropFArrow: return Attribute::Boolean(iHasFArrow);
    case EPropRArrow: return Attribute::Boolean(iHasRArrow);
    case EPropFArrowSize: return iFArrowSize;
    case EPropRArrowSize: return iRArrowSize;
    case EPropFArrowShape: return iFArrowShape;
    case EPropRArrowShape: return iRArrowShape;
    case EPropLineJoin: return Attribute(iLineJoin);
    case EPropLineCap: return Attribute(iLineCap);
    case EPropFillRule: return Attribute(iFillRule);
    default: return Object::getAttribute(prop);
    }
}
