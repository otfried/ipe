...
Bitmap::Bitmap(const XmlAttributes & attr, String pcdata) {
	...
	while (length-- > 0) *p++ = b64source.getChar();
	while (alphaLength-- > 0) *q++ = b64source.getChar();
    } else {
	Lex datalex(pcdata);
	while (length-- > 0) *p++ = char(datalex.getHexByte());
	while (alphaLength-- > 0) *q++ = char(datalex.getHexByte());
    }
    unpack(alpha);
    computeChecksum();
    analyze();
...
std::pair<int, int> Bitmap::init(const XmlAttributes & attr) {
    ...
    if (cs == "DeviceRGB") iImp->iFlags |= ERGB;
    ...
    String cc;
    if (!isJpeg() && attr.has("ColorKey", cc)) iImp->iColorKey = Lex(cc).getHexNumber();
...
void Bitmap::unpack(Buffer alphaChannel) {
	...
	if (alphaChannel.size() > 0) {
	    ...
	    assert(uncompress((Bytef *)inflatedAlpha.data(), &inflatedSize,
			      (const Bytef *)alphaChannel.data(), alphaChannel.size())
		   == Z_OK);
	...
	q = (uint32_t *)pixels.data();
	while (q < fin) {
	    if (*q == colorKey) *q = iImp->iColorKey;
	    ++q;
	}
    }
    iImp->iData = pixels;
...
Bitmap::Bitmap(const Bitmap & rhs) {
    iImp = rhs.iImp;
    if (iImp) iImp->iRefCount++;
}

//! Destructor.
Bitmap::~Bitmap() {
    if (iImp && --iImp->iRefCount == 0) { delete iImp; }
}
...
bool Bitmap::equal(Bitmap rhs) const {
    if (iImp == rhs.iImp) return true;
    if (!iImp || !rhs.iImp) return false;

    if (iImp->iFlags != rhs.iImp->iFlags || iImp->iWidth != rhs.iImp->iWidth
	|| iImp->iHeight != rhs.iImp->iHeight || iImp->iChecksum != rhs.iImp->iChecksum
...
void Bitmap::computeChecksum() { iImp->iChecksum = iImp->iData.checksum(); }
...
std::pair<Buffer, Buffer> Bitmap::getEmbedData() const {
	...
	p = alpha.data();
	while (src < fin) *p++ = (*src++ & 0xff000000) >> 24;
...
Buffer Bitmap::pixelData() {
	...
	if (isJpeg()) {
	    ...
	    if (!dctDecode(stream, pixels)) return Buffer();
...
const char * Bitmap::readJpegInfo(FILE * file, int & width, int & height,
    ...
    for (;;) {
	int ch = fgetc(file);
	if (ch != 0xff) return "Reading JPEG image failed";
	do { ch = fgetc(file); } while (ch == 0xff);
	...
	case M_SOF13:
	case M_SOF14:
	case M_SOF15: return "Unsupported type of JPEG compression";
	...
	case M_SOF3:
	    ...
	    ch = fgetc(file);
	    if (ch != 8) return "Unsupported bit width of pixels in JPEG image";
	    height = read2bytes(file);
	    width = read2bytes(file);
	    ch = fgetc(file);
	    switch (ch & 0xff) {
	    case JPG_GRAY: break;
	    case JPG_RGB: flags |= ERGB; break;
	    default: return "Unsupported color space in JPEG image";
	    }
	...
	case M_APP0: {
	    ...
	    for (int i = 0; i < 5; i++) {
		ch = fgetc(file);
		if (ch != jpg_id[i]) { return "Reading JPEG image failed"; }
	    }
	    ...
	    if (xres != 0 && yres != 0) {
		switch (units) {
		case 1: /* pixels per inch */ dotsPerInch = Vector(xres, yres); break;
