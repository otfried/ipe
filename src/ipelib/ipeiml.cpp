...
*/
Buffer ImlParser::pdfStream(int /* objNum */) { return Buffer(); }
...
int ImlParser::parseDocument(Document & doc) {
    ...
    if (tag == "?xml") {
	XmlAttributes attr;
	if (!parseAttributes(attr, true)) return ESyntaxError;
	tag = parseToTag();
    }
    if (tag != "ipe") return ESyntaxError;

    XmlAttributes attr;
    if (!parseAttributes(attr)) return ESyntaxError;

    Lex versLex(attr["version"]);
    int version;
    versLex >> version;
    if (version < OLDEST_FILE_FORMAT) return EVersionTooOld;
    if (version > IPELIB_VERSION) return EVersionTooRecent;
    ...
    if (tag == "info") {
	XmlAttributes att;
	if (!parseAttributes(att)) return ESyntaxError;
	...
	XmlAttributes att;
	if (!parseAttributes(att)) return ESyntaxError;
	if (!parsePCDATA("preamble", properties.iPreamble)) return ESyntaxError;
	...
	if (tag == "ipestyle") {
	    ...
	    if (!parseBitmap()) return ESyntaxError;
	}
	tag = parseToTag();
    }
	...
	Page * page = new Page;
	doc.push_back(page);
	if (!parsePage(*page)) return ESyntaxError;
	tag = parseToTag();
    }
...
bool ImlParser::parsePage(Page & page) {
    ...
    if (att["marked"] == "no") page.setMarked(false);
	...
	XmlAttributes att;
	if (!parseAttributes(att)) return false;
	if (!parsePCDATA("notes", str)) return false;
	page.setNotes(str);
	...
	if (att["edit"] == "no") page.setLocked(page.countLayers() - 1, true);
	String snapMode;
	...
	String data;
	if (att.has("data", data)) page.setLayerData(page.countLayers() - 1, data);
	tag = parseToTag();
    }
    // default layer: 'alpha'
    if (page.countLayers() == 0) page.addLayer("alpha");
	...
	if (att["marked"] == "yes") page.setMarkedView(page.countViews() - 1, true);

	if (att.has("name", str)) page.setViewName(page.countViews() - 1, str);

	if (!att.slash()) {
	    AttributeMap map;
	    if (!parseView(page, map)) return false;
	...
	int al = 0;
	while (al < page.countLayers() && page.isLocked(al)) ++al;
	if (al == page.countLayers()) return false; // no unlocked layer
...
Page * ImlParser::parsePageSelection() {
    String tag = parseToTag();
    if (tag != "ipepage") return nullptr;
    XmlAttributes attr;
    if (!parseAttributes(attr)) return nullptr;
    tag = parseToTag();

    while (tag == "bitmap") {
	if (!parseBitmap()) return nullptr;
	tag = parseToTag();
    }

    if (tag != "page") return nullptr;

    std::unique_ptr<Page> page(new Page);
    if (!parsePage(*page)) return nullptr;
...
Object * ImlParser::parseObject(String tag, Page * page, int * currentLayer) {
    String layer;
...
Object * ImlParser::parseObject(String tag, String & layer) {
    ...
    String l;
    if (attr.has("layer", l)) layer = l;

    if (tag == "group") {
	Group group(attr);
	for (;;) {
	    String tag = parseToTag();
	    if (tag == "/group") { return new Group(group); }
    ...
    String pcdata;
    if (!attr.slash() && !parsePCDATA(tag, pcdata)) return nullptr;
    String bitmapId;
...
bool ImlParser::parseStyle(StyleSheet & sheet) {
    ...
    if (!parseAttributes(att)) return false;
    String name;
    if (att.has("name", name)) sheet.setName(name);

    if (att.slash()) return true;

    String tag = parseToTag();
    while (tag != "/ipestyle") {
	if (tag == "bitmap") {
	    if (!parseBitmap()) return false;
	} else if (tag == "symbol") {
	    if (!parseAttributes(att)) return false;
		...
		if ((flags
		     & (Reference::EHasStroke | Reference::EHasFill | Reference::EHasPen
			| Reference::EHasSize))
		    == 0) {
		    symbol.iXForm = true;
		    symbol.iTransformations = ETransformationsTranslations;
	    ...
	    if (parseToTag() != "/symbol") return false;
	} else if (tag == "layout") {
	    if (!parseAttributes(att) || !att.slash()) return false;
	    Layout layout;
	    Lex lex1(att["paper"]);
	    ...
	    if (!att.slash() && !parsePCDATA(tag, pns.iText)) return false;
	    sheet.setPageNumberStyle(pns);
	} else if (tag == "preamble") {
	    if (!parseAttributes(att)) return false;
	    String pcdata;
	    if (!att.slash() && !parsePCDATA(tag, pcdata)) return false;
	    sheet.setPreamble(pcdata);
	} else if (tag == "pathstyle") {
	    if (!parseAttributes(att) || !att.slash()) return false;
	    String str;
	    if (att.has("cap", str)) sheet.setLineCap(TLineCap(Lex(str).getInt() + 1));
	    if (att.has("join", str)) sheet.setLineJoin(TLineJoin(Lex(str).getInt() + 1));
	    ...
	    if (!parseAttributes(att) || !att.slash()) return false;
	    String name = att["name"];
	    Attribute col = Attribute::makeColor(att["value"], Attribute::NORMAL());
	    if (!symbolName(name) || !col.isColor()) return false;
	    ...
	    if (!symbolName(name) || value.isSymbolic()) return false;
	    sheet.add(ETextSize, Attribute(true, name), value);
	} else if (tag == "textstretch") {
	    if (!parseAttributes(att) || !att.slash()) return false;
	    String name = att["name"];
	    Attribute value = Attribute::makeScalar(att["value"], Attribute::NORMAL());
	    if (!symbolName(name) || value.isSymbolic()) return false;
	    ...
	    if (!symbolName(name)) return false;
	    Gradient s;
	    s.iType = (att["type"] == "radial") ? Gradient::ERadial : Gradient::EAxial;
	    Lex lex(att["coords"]);
	    if (s.iType == Gradient::ERadial)
		lex >> s.iV[0].x >> s.iV[0].y >> s.iRadius[0] >> s.iV[1].x >> s.iV[1].y
		    >> s.iRadius[1];
	    else
		lex >> s.iV[0].x >> s.iV[0].y >> s.iV[1].x >> s.iV[1].y;
	    String str;
	    s.iExtend = (att.has("extend", str) && str == "yes");
	    if (att.has("matrix", str)) s.iMatrix = Matrix(str);
	    tag = parseToTag();
	    while (tag == "stop") {
		if (!parseAttributes(att) || !att.slash()) return false;
		Gradient::Stop st;
	    ...
	    if (s.iStops.size() < 2) return false;
		...
		if (s.iStops[i].offset < s.iStops[i - 1].offset) return false;
	    }
	    if (tag != "/gradient") return false;
	    ...
	    Effect s;
	    String str;
	    if (att.has("duration", str)) s.iDuration = Lex(str).getInt();
	    if (att.has("transition", str)) s.iTransitionTime = Lex(str).getInt();
	    if (att.has("effect", str)) s.iEffect = Effect::TEffect(Lex(str).getInt());
	    ...
	    String bg;
	    if (att.has("background", bg)) pageStyle.iBackground = Attribute(true, bg);
	    if (!att.slash()) {
		tag = parseToTag();
		while (tag == "map") {
		    if (!parseAttributeMapping(pageStyle.iMapping)) return false;
		    tag = parseToTag();
		}
		if (tag != "/pagestyle") return false;
	    }
	    ...
	    String name = att["name"];
	    Attribute value = Attribute::makeScalar(att["value"], Attribute::NORMAL());
	    if (name.empty() || value.isSymbolic()) return false;
...
StyleSheet * ImlParser::parseStyleSheet() {
    ...
    if (tag == "?xml") {
	XmlAttributes attr;
	if (!parseAttributes(attr, true)) return nullptr;
	tag = parseToTag();
    }
    if (tag != "ipestyle") return nullptr;
    StyleSheet * sheet = new StyleSheet();
    if (parseStyle(*sheet)) return sheet;
    delete sheet;
    return nullptr;
}
