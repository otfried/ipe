...
void Painter::setAttributeMap(const AttributeMap * map) { iAttributeMap = map; }
...
void Painter::transform(const Matrix & m) { iMatrix.back() = matrix() * m; }
...
void Painter::untransform(TTransformations trans) {
    if (trans == ETransformationsAffine) return;
    Matrix m = matrix();
...
void Painter::pushMatrix() { iMatrix.push_back(matrix()); }

//! Restore previous transformation matrix.
void Painter::popMatrix() { iMatrix.pop_back(); }
...
void Painter::dashStyle(std::vector<double> & dashes, double & offset) const {
    ...
    while (!lex.eos()) dashes.push_back(lex.getDouble());
...
void Painter::setLineCap(TLineCap cap) {
    assert(!iInPath);
    if (cap != EDefaultCap) iState.back().iLineCap = cap;
}
...
void Painter::setLineJoin(TLineJoin join) {
    assert(!iInPath);
    if (join != EDefaultJoin) iState.back().iLineJoin = join;
}
...
void Painter::setFillRule(TFillRule rule) {
    assert(!iInPath);
    if (rule != EDefaultRule) iState.back().iFillRule = rule;
}
...
void Painter::setTiling(Attribute tiling) {
    ...
    if (!tiling.isNormal()) iState.back().iGradient = Attribute::NORMAL();
}
...
void Painter::setGradient(Attribute grad) {
    ...
    if (!grad.isNormal()) iState.back().iTiling = Attribute::NORMAL();
}
...
void Painter::setState(const State & state) { iState.back() = state; }
...
void Painter::drawArcAsBezier(double alpha) {
	...
	if (divi == 0.0) return; // alpha2 is close to zero
