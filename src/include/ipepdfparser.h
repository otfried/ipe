...
public:
    explicit PdfName(const String & val)
	: iValue(val) { /* nothing */ }
    virtual const PdfName * name() const noexcept;
    virtual void write(Stream & stream, const PdfRenumber * renumber,
		       bool inflate) const noexcept;
...
public:
    explicit PdfDict()
	: iLateStreamPosition{0} { /* nothing */ }
    ~PdfDict();
    virtual const PdfDict * dict() const noexcept;
    String dictRepr() const noexcept;
    void dictWrite(Stream & stream, const PdfRenumber * renumber, bool inflate,
    ...
    bool getNumber(String key, double & val,
		   const PdfFile * file = nullptr) const noexcept;
    ...
    inline const PdfObj * value(int index) const noexcept { return iItems[index].iVal; }
