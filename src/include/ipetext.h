...
public:
    ...
    explicit Text();
    explicit Text(const AllAttributes & attr, String data, const Vector & pos,
		  TextType type, double width = 10.0);
    Text(const Text & rhs);
    ~Text();
...
inline Vector Text::position() const { return iPos; }

//! Return text source.
inline String Text::text() const { return iText; }

//! Return stroke color.
inline Attribute Text::stroke() const { return iStroke; }

//! Return font size.
inline Attribute Text::size() const { return iSize; }

//! Return Latex style of text object.
inline Attribute Text::style() const { return iStyle; }

//! Return width of text object.
inline double Text::width() const { return iWidth; }

//! Return height of text object (from baseline to top).
inline double Text::height() const { return iHeight; }

//! Return depth of text object.
inline double Text::depth() const { return iDepth; }

//! Return height + depth of text object.
inline double Text::totalHeight() const { return iHeight + iDepth; }

//! Return true if text object is formatted as a minipage.
/*! Equivalent to type being EMinipage. */
inline bool Text::isMinipage() const { return (iType == EMinipage); }
...
inline TVerticalAlignment Text::verticalAlignment() const { return iVerticalAlignment; }

//! Return the PDF representation of this text object.
/*! If Pdflatex has not been run yet, returns 0. */
inline const Text::XForm * Text::getXForm() const { return iXForm; }
