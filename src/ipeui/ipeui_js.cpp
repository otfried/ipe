...
Dialog::Result PDialog::buildAndRun(int w, int h) {
    ...
    for (int s : iRowStretch) rowstretch.call<void>("push", s);
    val colstretch = val::array();
    for (int s : iColStretch) colstretch.call<void>("push", s);
    iOptions = val::object();
...
static int ipeui_fileDialog(lua_State * L) {
    ...
    val dir = val::null();
    if (!lua_isnoneornil(L, 5)) dir = val(checkstring(L, 5));
    val path = val::null();
    if (!lua_isnoneornil(L, 6)) path = val(checkstring(L, 6));
    int selected = 0;
...
static int ipeui_messageBox(lua_State * L) {
    ...
    std::string details;
    if (!lua_isnoneornil(L, 4)) details = checkstring(L, 4);
    int buttons = 0;
    if (lua_isnumber(L, 5))
...
static int ipeui_currentDateTime(lua_State * L) {
    ...
    char buf[15];
    sprintf(buf, "%04d%02d%02d%02d%02d%02d", dt.call<int>("getFullYear"),
	    dt.call<int>("getMonth") + 1, dt.call<int>("getDate"),
	    dt.call<int>("getHours"), dt.call<int>("getMinutes"),
