...
void PDialog::retrieveValues() {
	...
	switch (m.type) {
	case EInput: m.text = String(gtk_entry_get_text(GTK_ENTRY(w))); break;
	...
	case ECombo: m.value = gtk_combo_box_get_active(GTK_COMBO_BOX(w)); break;
	case ECheckBox:
...
static GtkWidget * addScrollBar(GtkWidget * w) {
    ...
    gtk_scrolled_window_set_shadow_type(GTK_SCROLLED_WINDOW(ww), GTK_SHADOW_ETCHED_IN);
...
Dialog::Result PDialog::buildAndRun(int w, int h) {
	...
	if (m.flags & EDisabled) gtk_widget_set_sensitive(w, false);
...
private:
    ...
    static void positionResponse(GtkMenu *, gint * x, gint * y, gboolean * push_in,
				 PMenu * menu);
...
void PMenu::positionResponse(GtkMenu *, gint * x, gint * y, gboolean * push_in,
			     PMenu * menu) {
    *x = menu->iPopupX;
    *y = menu->iPopupY;
    *push_in = TRUE;
...
PMenu::PMenu(WINID parent) {
    ...
    g_signal_connect(iMenu, "deactivate", G_CALLBACK(deactivateResponse), this);
}
...
static int ipeui_fileDialog(lua_State * L) {
    ...
    if (!lua_isnoneornil(L, 5)) dir = luaL_checkstring(L, 5);
    const char * name = nullptr;
    if (!lua_isnoneornil(L, 6)) name = luaL_checkstring(L, 6);
...
static int ipeui_messageBox(lua_State * L) {
    ...
    GtkWidget * dlg = gtk_message_dialog_new(GTK_WINDOW(parent), GTK_DIALOG_MODAL, t,
    ...
    case GTK_RESPONSE_OK: lua_pushnumber(L, 1); break;
    case GTK_RESPONSE_NO: lua_pushnumber(L, 0); break;
