...
WINID check_winid(lua_State * L, int i) {
    if (lua_isnil(L, i)) return nullptr;
...
static const struct luaL_Reg winid_methods[] = {{"__tostring", winid_tostring},
						{nullptr, nullptr}};
...
int Dialog::addButton(lua_State * L) {
    ...
    if (m.minWidth < 64) m.minWidth = 64;
...
int Dialog::add(lua_State * L) {
    ...
    if (m.row < 0) m.row = iNoRows + 1 + m.row;
    m.col = luaL_checkinteger(L, 6) - 1;
    m.rowspan = 1;
    m.colspan = 1;
    if (!lua_isnoneornil(L, 7)) m.rowspan = luaL_checkinteger(L, 7);
    if (!lua_isnoneornil(L, 8)) m.colspan = luaL_checkinteger(L, 8);
    if (m.row + m.rowspan > iNoRows) iNoRows = m.row + m.rowspan;
    if (m.col + m.colspan > iNoCols) iNoCols = m.col + m.colspan;

    switch (m.type) {
    case EButton: addButtonItem(L, m); break;
    case ETextEdit: addTextEdit(L, m); break;
    case EList: addList(L, m); break;
    case ELabel: addLabel(L, m); break;
...
void Dialog::addInput(lua_State * L, SElement & m) {
    ...
    if (lua_toboolean(L, -1)) m.flags |= ESelectAll;
    lua_getfield(L, 4, "focus");
    if (lua_toboolean(L, -1)) m.flags |= EFocused;
...
void Dialog::addTextEdit(lua_State * L, SElement & m) {
    ...
    if (lua_toboolean(L, -1)) m.flags |= EReadOnly;
    lua_getfield(L, 4, "select_all");
    if (lua_toboolean(L, -1)) m.flags |= ESelectAll;
    lua_getfield(L, 4, "focus");
    if (lua_toboolean(L, -1)) m.flags |= EFocused;
    ...
    if (lua_toboolean(L, -1)) m.flags |= ESpellCheck;
...
int Dialog::findElement(lua_State * L, int index) {
	...
	if (name == iElements[i].name) return i;
    }
...
int Dialog::set(lua_State * L) {
    ...
    if (iLuaDialog != LUA_NOREF) setMapped(L, idx);
    return 0;
}
...
void Dialog::setUnmapped(lua_State * L, int idx) {
	...
	break;
    case ECheckBox: m.value = lua_toboolean(L, 3); break;
    default: luaL_argerror(L, 2, "no suitable element");
    }
}

int Dialog::get(lua_State * L) {
    if (iLuaDialog != LUA_NOREF) retrieveValues();
    ...
    switch (m.type) {
    case ETextEdit:
    case EInput: lua_pushstring(L, m.text.c_str()); return 1;
    case EList:
    case ECombo: lua_pushinteger(L, m.value + 1); return 1;
    case ECheckBox: lua_pushboolean(L, m.value); return 1;
    default: return luaL_argerror(L, 2, "no suitable element");
    }
}
...
int Dialog::setStretch(lua_State * L) {
    ...
    if (type == 0) {
	luaL_argcheck(L, 0 <= rowcol && rowcol < iNoRows, 3, "Row index out of range");
	iRowStretch[rowcol] = stretch;
    } else {
	luaL_argcheck(L, 0 <= rowcol && rowcol < iNoCols, 3, "Column index out of range");
...
static const struct luaL_Reg timer_methods[] = {{"__tostring", timer_tostring},
...
static void make_metatable(lua_State * L, const char * name,
	...
	if (ok != LUA_OK) luaL_error(L, "cannot prepare m:execute function");
	lua_call(L, 0, 1);
	lua_setfield(L, -2, "execute");
    }
#endif
