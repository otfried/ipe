...
static INT_PTR CALLBACK dialogProc(HWND hwnd, UINT message, WPARAM wParam,
    ...
    case WM_CLOSE: EndDialog(hwnd, -1); return (INT_PTR)TRUE;
    default: return (INT_PTR)FALSE;
    }
}

int showPageSelectDialog(int width, int height, const char * title, HIMAGELIST il,
			 std::vector<String> & labels, int startIndex) {
    SData sData;
    sData.hImageList = il;
    sData.startIndex = startIndex;
    for (int i = 0; i < size(labels); ++i) sData.labels.push_back(labels[i].w());
    ...
    buildFlags(t, WS_CHILD | WS_VISIBLE | LVS_ICON | LVS_SHAREIMAGELISTS | WS_VSCROLL
...
int CanvasBase::selectPageOrView(Document * doc, int page, int startIndex, int pageWidth,
    ...
    HIMAGELIST il = ImageList_Create(pageWidth, r.height(), ILC_COLOR32, nItems, 4);
    if (page >= 0) {
