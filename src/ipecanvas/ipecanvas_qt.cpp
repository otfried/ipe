...
void IpeQtPainter::doNewPath() { iPP = QPainterPath(); }

void IpeQtPainter::doMoveTo(const Vector & v) { iPP.moveTo(QPt(v)); }

void IpeQtPainter::doLineTo(const Vector & v) { iPP.lineTo(QPt(v)); }

void IpeQtPainter::doCurveTo(const Vector & v1, const Vector & v2, const Vector & v3) {
    iPP.cubicTo(QPt(v1), QPt(v2), QPt(v3));
}

void IpeQtPainter::doClosePath() { iPP.closeSubpath(); }
...
void Canvas::setCursor(TCursor cursor, double w, Color * color) {
    ...
    default: QWidget::unsetCursor(); break;
    case EHandCursor: QWidget::setCursor(QCursor(Qt::PointingHandCursor)); break;
    case ECrossCursor: QWidget::setCursor(QCursor(Qt::CrossCursor)); break;
    case EDotCursor: {
	QPixmap p(32, 32);
...
void Canvas::invalidate() { QWidget::update(); }
...
static int convertModifiers(int qmod) {
    int mod = 0;
    if (qmod & Qt::ShiftModifier) mod |= CanvasBase::EShift;
    if (qmod & Qt::ControlModifier) mod |= CanvasBase::EControl;
    if (qmod & Qt::AltModifier) mod |= CanvasBase::EAlt;
    if (qmod & Qt::MetaModifier) mod |= CanvasBase::EMeta;
    return mod;
}
...
void Canvas::mouseDoubleClickEvent(QMouseEvent * ev) { mouseButton(ev, 0x81, true); }

void Canvas::mousePressEvent(QMouseEvent * ev) { mouseButton(ev, ev->button(), true); }

void Canvas::mouseReleaseEvent(QMouseEvent * ev) { mouseButton(ev, ev->button(), false); }
...
void Canvas::tabletEvent(QTabletEvent * ev) {
	...
	ipeDebug("TabletPress: %d %d %ld", ev->button(), ev->pointerType(),
		 ev->timestamp());
	pressed = true;
	ev->accept();
	...
	if (ev->pointerType() == QPointingDevice::PointerType::Eraser) {
	    if (iObserver)
		iObserver->canvasObserverMouseAction(Qt::XButton1 | iAdditionalModifiers);
	} else if (iTool)
	    iTool->mouseButton(ev->button() | iAdditionalModifiers, true);
	else if (iObserver)
	    iObserver->canvasObserverMouseAction(ev->button() | iAdditionalModifiers);
	break;
	...
	iGlobalPos = globalPos;
	computeFifi(hiPos.x(), hiPos.y());
	if (iTool) iTool->mouseButton(Qt::LeftButton, false);
	break;
    default: ipeDebug("Unknown tablet event"); break;
    }
}
...
static void draw_x(const Vector & p, QPainter & q) {
    q.drawLine(QPt(p - Vector(5.6, 5.6)), QPt(p + Vector(5.6, 5.6)));
    q.drawLine(QPt(p - Vector(5.6, -5.6)), QPt(p + Vector(5.6, -5.6)));
}
...
void Canvas::paintEvent(QPaintEvent * ev) {
    ...
    QImage bits(cairo_image_surface_get_data(iSurface), iBWidth, iBHeight,
		QImage::Format_RGB32);
    qPainter.drawImage(r, bits, source);
    if (iFifiVisible) drawFifi(qPainter);
    if (iPage) {
