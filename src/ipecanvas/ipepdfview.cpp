...
void PdfViewBase::setBackground(const Color & bg) { iBackground = bg; }

//! Set blackout
void PdfViewBase::setBlackout(bool bo) { iBlackout = bo; }
...
void PdfViewBase::setPan(const Vector & v) { iPan = v; }

//! Set current zoom factor.
/*! The zoom factor maps user coordinates to screen pixel coordinates. */
void PdfViewBase::setZoom(double zoom) { iZoom = zoom; }
...
void PdfViewBase::drawPaper(cairo_t * cc) {
	...
	cairo_rectangle(cc, iPaperBox.left(), iPaperBox.bottom(), iPaperBox.width(),
			iPaperBox.height());
