...
typedef WINBOOL WINAPI (*LPSetGestureConfig)(HWND hwnd, DWORD dwReserved, UINT cIDs,
...
void Canvas::invalidate() { InvalidateRect(hwnd, nullptr, FALSE); }
...
void Canvas::key(WPARAM wParam, LPARAM lParam) {
    int mod = 0;
    if (GetKeyState(VK_SHIFT) < 0) mod |= EShift;
    if (GetKeyState(VK_CONTROL) < 0) mod |= EControl;
    if (GetKeyState(VK_MENU) < 0) mod |= EAlt;
...
void Canvas::mouseButton(Canvas * canvas, int button, bool down, WPARAM wParam,
			 LPARAM lParam) {
    if (canvas) {
	int xPos = GET_X_LPARAM(lParam);
	int yPos = GET_Y_LPARAM(lParam);
	int mod = 0;
	if (wParam & MK_SHIFT) mod |= EShift;
	if (wParam & MK_CONTROL) mod |= EControl;
	if (GetKeyState(VK_MENU) < 0) mod |= EAlt;
...
Vector Canvas::location(HWND hwnd, POINT q, POINT h) {
    ...
    return Vector(h.x / iHimetric.x + p.x - q.x, h.y / iHimetric.y + p.y - q.y);
}

LRESULT Canvas::handlePointerEvent(HWND hwnd, UINT message, WPARAM wParam,
				   LPARAM lParam) {
...
LRESULT CALLBACK Canvas::wndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	if (canvas) SetFocus(canvas->hwnd);
	return MA_ACTIVATE;
    case WM_KEYDOWN:
	if (lParam & 0x1000000) {
	...
	if (canvas && canvas->iObserver)
		...
		0, GET_WHEEL_DELTA_WPARAM(wParam) / 8.0, ((wParam & MK_CONTROL) ? 2 : 0));
	return 0;
    case WM_MOUSEHWHEEL:
	...
	if (gi.dwID == GID_ZOOM)
	    return canvas->handleZoomGesture(gi.dwFlags, gi.ptsLocation, gi.ullArguments);
	break;
    }
    case WM_MOUSEMOVE:
	if (canvas) {
	...
	break;
    case WM_LBUTTONDBLCLK: mouseButton(canvas, 0x81, true, wParam, lParam); break;
    case WM_RBUTTONDOWN:
    case WM_RBUTTONUP:
	...
	ipeDebug("Canvas MINMAX %ld %ld", mm->ptMinTrackSize.x, mm->ptMinTrackSize.y);
	break;
    }
    case WM_POINTERDOWN:
    case WM_POINTERUP:
...
Canvas::Canvas(HWND parent, HINSTANCE hInstance)
    : iHimetric(0., 0.) {
	...
	DWORD panWant = GC_PAN | GC_PAN_WITH_SINGLE_FINGER_VERTICALLY
			| GC_PAN_WITH_SINGLE_FINGER_HORIZONTALLY | GC_PAN_WITH_GUTTER
			| GC_PAN_WITH_INERTIA;
	DWORD panBlock = 0;
	GESTURECONFIG gestureConfig[] = {{GID_PAN, panWant, panBlock},
					 {GID_ZOOM, GC_ZOOM, 0},
...
void Canvas::init(HINSTANCE hInstance) {
    ...
    if (hDll) {
	pSetGestureConfig = (LPSetGestureConfig)GetProcAddress(hDll, "SetGestureConfig");
	pGetGestureInfo = (LPGetGestureInfo)GetProcAddress(hDll, "GetGestureInfo");
	pCloseGestureInfoHandle =
	    (LPCloseGestureInfoHandle)GetProcAddress(hDll, "CloseGestureInfoHandle");
	pGetPointerType = (LPGetPointerType)GetProcAddress(hDll, "GetPointerType");
	pGetPointerPenInfo =
	    (LPGetPointerPenInfo)GetProcAddress(hDll, "GetPointerPenInfo");
	pGetPointerPenInfoHistory =
	    (LPGetPointerPenInfoHistory)GetProcAddress(hDll, "GetPointerPenInfoHistory");
	pGetDpiForWindow = (LPGetDpiForWindow)GetProcAddress(hDll, "GetDpiForWindow");
    }
}
