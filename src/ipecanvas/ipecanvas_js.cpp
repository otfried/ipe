...
public:
    ...
    void drawPath(const Vector & v1, const Vector & v2, const Vector & v3,
		  const Vector & v4);
...
void JsPainter::drawPath(const Vector & v1, const Vector & v2, const Vector & v3,
			 const Vector & v4) {
    iCtx.call<void>("beginPath");
    iCtx.call<void>("moveTo", v1.x * iDpr, v1.y * iDpr);
    iCtx.call<void>("lineTo", v2.x * iDpr, v2.y * iDpr);
...
void JsPainter::doNewPath() { iCtx.call<void>("beginPath"); }

void JsPainter::doMoveTo(const Vector & v) { iCtx.call<void>("moveTo", v.x, v.y); }

void JsPainter::doLineTo(const Vector & v) { iCtx.call<void>("lineTo", v.x, v.y); }
...
void JsPainter::doClosePath() { iCtx.call<void>("closePath"); }
...
EM_JS(void, addIpeCanvasJS, (), {
	...
	const buffer2 =
	    new Uint8ClampedArray(buffer1.buffer, buffer1.byteOffset, buffer1.byteLength);
...
Canvas::Canvas(val bottomCanvas, val topCanvas) {
    ...
    ipeDebug("Canvas has size: %g x %g (%g x %g)", iWidth, iHeight, iBWidth, iBHeight);
}
...
void Canvas::invalidate(int x, int y, int w, int h) { invalidate(); }

// --------------------------------------------------------------------

static int convertModifiers(val ev) {
    int mod = 0;
    if (ev["shiftKey"].as<bool>()) mod |= CanvasBase::EShift;
    if (ev["ctrlKey"].as<bool>()) mod |= CanvasBase::EControl;
    if (ev["altKey"].as<bool>()) mod |= CanvasBase::EAlt;
    if (ev["metaKey"].as<bool>()) mod |= CanvasBase::EMeta;
    return mod;
}
...
void Canvas::mouseButtonEvent(emscripten::val ev, int button, bool press) {
    ...
    // ipeDebug("mouseButton %g, %g %d %d %d", iUnsnappedMousePos.x, iUnsnappedMousePos.y,
    // press, button, mod);
    if (iTool)
...
static void draw_rhombus(const Vector & p, JsPainter & q) {
    q.drawPath(p - Vector(8, 0), p + Vector(0, 8), p + Vector(8, 0), p + Vector(0, -8));
}

static void draw_square(const Vector & p, JsPainter & q) {
    q.drawPath(p + Vector(-7, -7), p + Vector(7, -7), p + Vector(7, 7),
	       p + Vector(-7, 7));
}

static void draw_x(const Vector & p, JsPainter & q) {
    q.drawLine(p - Vector(5.6, 5.6), p + Vector(5.6, 5.6));
    q.drawLine(p - Vector(5.6, -5.6), p + Vector(5.6, -5.6));
}
...
void Canvas::paint() {
	...
	int nPixels = cairo_image_surface_get_width(iSurface)
	...
	val buffer1 =
	    val(typed_memory_view(nPixels * 4, cairo_image_surface_get_data(iSurface)));
