...
void PanTool::draw(Painter & painter) const {
	...
	if (iPage->objectVisible(iView, i)) iPage->object(i)->drawSimple(painter);
    }
}
...
class SelectCompare {
public:
    int operator()(const SelectTool::SObj & lhs, const SelectTool::SObj & rhs) const {
	return (lhs.distance < rhs.distance);
    }
};

//! Constructor starts selection.
SelectTool::SelectTool(CanvasBase * canvas, Page * page, int view, double selectDistance,
		       bool nonDestructive)
    : Tool(canvas) {
    iPage = page;
    iView = view;
...
void SelectTool::mouseButton(int button, bool press) {
	...
	if (iNonDestructive) {
		...
		if (old_primary >= 0) iPage->setSelect(old_primary, ESecondarySelected);
	...
	if (iNonDestructive) {
	    ...
	    if (iPage->select(index) != EPrimarySelected) changed = true;
	    iPage->setSelect(index, EPrimarySelected);
	}
    } else {
...
void SelectTool::mouseMove() {
    ...
    if ((iCorner - iMouseDown).sqLen() > 9.0) iDragging = true;
    iCanvas->updateTool();
}
...
TransformTool::TransformTool(CanvasBase * canvas, Page * page, int view, TType type,
			     bool withShift)
    : Tool(canvas) {
    iPage = page;
    iView = view;
    ...
    if (iType == ETranslate) iCanvas->setAutoOrigin(iMouseDown);
	...
	if (page->select(i)) pin = TPinned(pin | page->object(i)->pinned());
    }
	...
	Rect bbox;
	for (int i = 0; i < iPage->count(); ++i) {
	    if (iPage->select(i)) bbox.addRect(iPage->bbox(i));
	}
...
void TransformTool::compute(const Vector & v1) {
    ...
    case ERotate:
	iTransform =
	    (Matrix(iOrigin) * Linear(Angle(u1.angle() - u0.angle())) * Matrix(-iOrigin));
	break;
    case EScale: {
	double factor = sqrt(u1.sqLen() / u0.sqLen());
	iTransform = (Matrix(iOrigin) * Linear(factor, 0, 0, factor) * Matrix(-iOrigin));
	break;
    }
    case EStretch: {
	...
	Matrix m = (Matrix(iOrigin) * Linear(iDir) * Linear(xfactor, 0, 0, yfactor) * rot
		    * Matrix(-iOrigin));
	if (std::fabs(m.determinant()) > 0.0001) iTransform = m;
	break;
    }
    case EShear: {
	auto rot = Linear(-iDir);
	...
	Matrix m = (Matrix(iOrigin) * Linear(iDir) * Linear(1.0, 0, s, 1.0) * rot
		    * Matrix(-iOrigin));
	if (std::fabs(m.determinant()) > 0.0001) iTransform = m;
	break;
    }
