...
static const char * const set_names[] = {"preamble", "linecap",    "linejoin",
					 "fillrule", "symbol",     "layout",
...
static int sheet_addfrom(lua_State * L) {
	...
	if (!g) luaL_argerror(L, 4, "no such gradient");
	s->addGradient(sym, *g);
	break;
    }
    case EEffect: {
	const Effect * e = t->findEffect(sym);
	if (!e) luaL_argerror(L, 4, "no such effect");
	s->addEffect(sym, *e);
	break;
    }
    case ETiling: {
	const Tiling * g = t->findTiling(sym);
	if (!g) luaL_argerror(L, 4, "no such tiling");
	s->addTiling(sym, *g);
	break;
    }
    default: luaL_argerror(L, 3, "cannot handle this kind"); break;
    }
    return 0;
}
...
static int sheet_set(lua_State * L) {
	...
	break;
    default: luaL_argerror(L, 2, "invalid kind for 'set'"); break;
    }
    return 0;
}
...
static int cascade_insert(lua_State * L) {
    Cascade * p = check_cascade(L, 1)->cascade;
    int index = (int)luaL_checkinteger(L, 2);
    luaL_argcheck(L, 1 <= index && index <= p->count() + 1, 2, "index out of bounds");
    SSheet * s = check_sheet(L, 3);
    StyleSheet * sheet = s->sheet;
    if (!s->owned) sheet = new StyleSheet(*s->sheet);
...
static const struct luaL_Reg cascade_methods[] = {{"__gc", cascade_destructor},
