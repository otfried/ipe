...
static int check_objno(lua_State * L, int i, Page * p, int extra = 0) {
    ...
    luaL_argcheck(L, 1 <= n && n <= p->count() + extra, i, "invalid object index");
    return n - 1;
}
...
int ipelua::check_viewno(lua_State * L, int i, Page * p, int extra) {
    int n = (int)luaL_checkinteger(L, i);
    luaL_argcheck(L, 1 <= n && n <= p->countViews() + extra, i, "invalid view index");
    return n - 1;
}
...
static int page_xml(lua_State * L) {
    static const char * const option_names[] = {"ipepage", "ipeselection", nullptr};
    Page * p = check_page(L, 1)->page;
...
static const char * const snapmode_names[] = {"never", "visible", "always", nullptr};
...
static int page_moveLayer(lua_State * L) {
    ...
    luaL_argcheck(L, 0 <= newIndex && newIndex < p->countLayers(), 3,
...
static const Kind map_kind_kinds[] = {EPen,   ESymbolSize, EArrowSize, EOpacity,
				      EColor, EDashStyle,  ESymbol};
...
static AttributeMap check_attribute_map(lua_State * L, int index) {
	...
	if (!lua_isstring(L, -1)) luaL_error(L, "element %d has no kind", i + 1);
	int k = test_option(L, -1, map_kind_names);
	if (k < 0) luaL_error(L, "element %d has invalid kind", i + 1);
...
static int page_setLayerMatrices(lua_State * L) {
	...
	if (layerNum < 0) luaL_error(L, "page has no such layer");
...
static int page_findedge(lua_State * L) {
    ...
    Snap snap;
    if (!snap.setEdge(pos, p, view)) return 0;
