...
int Parser::parseDocument() {
	...
	if (!parsePCDATA("preamble", str)) return false;
	iStream << "<preamble>";
    ...
    if (needMediaSheet) {
	iStream << "<ipestyle>\n";
	iStream << "<layout paper=\"" << paper << "\" origin=\"0 0\" frame=\"" << paper
		<< "\"/>\n";
	iStream << "</ipestyle>\n";
    }
	...
	if (tag == "ipestyle") {
	    if (!parseStyle()) return ESyntaxError;
	} else { // tag == "bitmap"
    ...
    while (tag == "page") {
	if (!parsePage()) return ESyntaxError;
	tag = parseToTag();
    }
...
bool Parser::parsePage() {
	...
	if (!parseObject(tag, a)) return false;
	tag = parseToTag();
    }
    return true;
}
...
bool Parser::parseGroup(const XmlAttributes & attr, const Attributes & a) {
    ...
    Attributes a1 = a;
    if (a.textsize.empty()) a1.textsize = attr["textsize"];
    if (a.marksize.empty()) a1.marksize = attr["marksize"];
    if (a.markshape.empty()) a1.markshape = attr["markshape"];
    if (a.stroke.empty()) a1.stroke = attr["stroke"];
    if (a.fill.empty()) a1.fill = attr["fill"];
    if (a.dashStyle.empty()) a1.dashStyle = attr["dash"];
    if (a.pen.empty()) a1.pen = attr["pen"];
    if (a.cap.empty()) a1.cap = attr["cap"];
    if (a.join.empty()) a1.join = attr["join"];
    if (a.fillrule.empty()) a1.fillrule = attr["fillrule"];
	...
	if (!parseObject(tag, a1)) return false;
	tag = parseToTag();
    }
}

static const char * const marktypes[] = {"circle(sx)",  "disk(sx)",  "box(sx)",
					 "square(sx)",  "cross(sx)", "fdisk(sfx)",
					 "fsquare(sfx)"};
...
bool Parser::parseObject(String tag, const Attributes & a) {
    ...
    if (tag == "group") return parseGroup(attr, a);

    String pcdata;
    if (!attr.slash() && !parsePCDATA(tag, pcdata)) return false;
	...
	if (dash.empty()) dash = attr["dash"];
	String stroke = a.stroke;
	if (stroke.empty()) stroke = attr["stroke"];
	if (dash != "void" && stroke != "void") {
	    writeAttr(attr, "stroke", a.stroke);
	    // in Ipe7 "solid" is "normal" and is the default anyway
	    if (dash != "" and dash != "solid") iStream << " dash=\"" << dash << "\"";
	...
	if (!rarrow.empty()) iStream << " rarrow=\"" << rarrow << "\"";
	writeAttr(attr, "fill", a.fill);
...
static const char * const styledefs[] = {
    "titlestyle", "layout",   "textstyle",   "pathstyle", "color",
    "dashstyle",  "textsize", "textstretch", "marksize",  "arrowsize"};
...
bool Parser::parseStyle() {
	...
	if (tag == "bitmap") {
	    ...
	    Attributes a;
	    if (!parseObject(tag1, a)) return false;
	    if (parseToTag() != "/template") return false;
	    iStream << "</symbol>\n";
	...
	} else if (tag == "textmatrix" || tag == "media" || tag == "margins"
		   || tag == "shading") {
	    ...
	    if (!parseAttributes(att) || !att.slash()) return false;
	    if (!iUseCropBox) att.add("crop", "no");
	    writeTag("layout", att);
	} else {
	    // only standard tags remain
	    size_t i = 0;
	    while (i < (sizeof(styledefs) / sizeof(const char *)) && tag != styledefs[i])
		++i;
	    if (i == (sizeof(styledefs) / sizeof(const char *))) return false;
	    if (!parseAttributes(att) || !att.slash()) return false;
	    if (tag == "color") {
		if (att["value"] == "black")
		    att.add("value", "0");
...
static void usage() {
    fprintf(stderr, "Usage: ipe6upgrade <input.xml> [ <output.ipe> ]\n"
